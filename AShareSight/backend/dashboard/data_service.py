...
def fetch_snapshot(symbol: str, asset_type: str) -> dict[str, Any] | None:
    """Return snapshot dict (A-share: rqdatac / Eastmoney first)."""
    market = _infer_equity_market(symbol)
    if market == "CN" and asset_type == "equity":
        try:
            ...
            obid = to_rqdatac_id(symbol) or symbol
            q = get_stock_price(obid)
            info = get_company_info(obid)
            last = safe_float(q.get("price")) if isinstance(q, dict) else None
            output: dict[str, Any] = {
                "currency": "CNY",
                "last_price": last,
                "eps": safe_float((info or {}).get("pe_ttm")) if isinstance(info, dict) else None,
            }
            if isinstance(info, dict):
                output["market_cap"] = safe_float(info.get("market_cap"))
                output["pe_ttm"] = safe_float(info.get("pe_ttm"))
            return output
        except Exception as exc:
            logger.warning("[DataService] CN snapshot rqdatac failed for %s: %s", symbol, exc)
...
def _infer_equity_market(symbol: str) -> str:
    ...
    if ticker.endswith((".SS", ".SZ", ".BJ", ".XSHG", ".XSHE", ".XBEX")):
        return "CN"
...
# In-memory OHLCV DataFrame cache (60s TTL) to avoid duplicate yfinance calls
# when market_chart and technicals both need the same data in one request.
_OHLCV_FRAME_CACHE: dict[str, dict] = {}
...
def _load_ohlcv_frame(symbol: str, period: str = "1y", interval: str = "1d") -> Optional[pd.DataFrame]:
    ...
    Results are cached in-memory for 60s to avoid duplicate yfinance calls
    (e.g. when market_chart and technicals both need the same OHLCV data).
    """
    import time as _time
    _cache_key = f"{symbol}:{period}:{interval}"
    _now = _time.time()
    _entry = _OHLCV_FRAME_CACHE.get(_cache_key)
    if _entry is not None and _now < _entry["expires_at"]:
        return _entry["frame"]

    def _cache_result(frame: pd.DataFrame) -> pd.DataFrame:
        _OHLCV_FRAME_CACHE[_cache_key] = {"frame": frame, "expires_at": _time.time() + 60}
        return frame

    market = _infer_equity_market(symbol)
    if market == "CN":
        try:
            from backend.tools.rqdata_price import get_stock_historical_data
            from backend.utils.ticker_rq import to_rqdatac_id

            hist = get_stock_historical_data(to_rqdatac_id(symbol) or symbol, period=period, interval=interval)
            if isinstance(hist, dict):
                rows = hist.get("rows") or []
                frame = _build_ohlcv_frame_from_rows(rows)
                if frame is not None and not frame.empty:
                    return _cache_result(frame)
        except Exception as exc:
            logger.warning("[DataService] rqdatac OHLCV failed for %s: %s", symbol, exc)
    ...
    if market in {"CN", "HK"}:
        try:
            ...
            short = from_rqdatac_id(to_rqdatac_id(symbol) or symbol)
            cn_hk_rows = fetch_cn_hk_kline(short, limit=300)
    ...
    try:
        ...
        payload = get_stock_historical_data(to_rqdatac_id(symbol) or symbol, period=period, interval=interval)
        if not isinstance(payload, dict):
            return None
        rows = payload.get("rows") or payload.get("kline_data") or []
