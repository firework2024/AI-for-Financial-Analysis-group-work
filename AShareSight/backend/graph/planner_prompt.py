...
def build_planner_prompt(state: GraphState, variant: str = "A") -> str:
    ...
    return f"""<role>AShareSight Planner — 负责为 A 股金融助手创建最优执行计划</role>

<market_rules>
- 标的为 A 股，代码格式如 600519.XSHG / 000001.XSHE；货币 CNY（¥）。
- 交易时间：北京时间 9:30-11:30、13:00-15:00；T+1；涨跌停（主板±10%、科创/创业±20%、北交±30%）。
- 数据源优先 rqdatac（米筐），不可用时报 eastmoney；勿规划美股/期权/ETF 专用工具。
</market_rules>
