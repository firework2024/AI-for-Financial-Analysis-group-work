...
def _extract_price(output: Any) -> dict[str, Any]:
    ...
    if parsed_quote:
        parsed_quote["currency"] = "CNY"
        if isinstance(parsed, dict):
            data = parsed.get("data") if isinstance(parsed.get("data"), dict) else parsed
            parsed_quote["currency"] = data.get("currency") or data.get("financialCurrency") or "CNY"
    ...
    currency = data.get("currency") or data.get("financialCurrency") or "CNY"
...
def _format_price_line(ticker: str, price: dict[str, Any]) -> str:
    ...
    parts = [f"{ticker} 最新价格约为 {_format_number(price['price'])} {price.get('currency') or 'CNY'}"]
