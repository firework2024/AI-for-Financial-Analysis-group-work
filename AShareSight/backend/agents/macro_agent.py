...
class MacroAgent(BaseFinancialAgent):
    ...
    - Execute: collect from China macro (rqdatac.econ), official releases, search
    ...
    _INDICATORS: Dict[str, Dict[str, str]] = {
        "lpr": {"label": "LPR 贷款市场报价利率", "unit": "%"},
        "pmi": {"label": "制造业 PMI", "unit": "点"},
        "m2": {"label": "M2 同比增速", "unit": "%"},
        "cpi": {"label": "CPI 同比", "unit": "%"},
        "ppi": {"label": "PPI 同比", "unit": "%"},
        "gdp": {"label": "GDP 增速", "unit": "%"},
        "shibor": {"label": "Shibor", "unit": "%"},
        "bond_10y": {"label": "10年期国债收益率", "unit": "%"},
        "social_financing": {"label": "社融", "unit": "万亿"},
    }
    ...
    def _get_tool_registry(self) -> dict:
        ...
        macro_fn = getattr(tools, "get_china_macro_snapshot", None) or getattr(tools, "get_fred_data", None)
        if macro_fn:
            registry["get_china_macro_snapshot"] = {
                "func": macro_fn,
                "description": "获取中国宏观指标（M2/CPI/PPI/PMI/准备金率等）",
                "call_with": "none",
            }
    ...
    async def _initial_search(self, query: str, ticker: str) -> Dict[str, Any]:
        ...
        china_metrics: Dict[str, float] = {}
        china_payload: Dict[str, Any] = {}
        try:
            macro_fn = getattr(self.tools, "get_china_macro_snapshot", None) or getattr(self.tools, "get_fred_data", None)
            if macro_fn:
                payload = macro_fn()
                if isinstance(payload, dict):
                    china_payload = payload
                    china_metrics = self._extract_numeric_metrics(payload.get("indicators") or payload)
                    if china_metrics:
                        source_health["rqdatac_econ"] = "ok"
                        used_sources.append("rqdatac_econ")
                    else:
                        source_health["rqdatac_econ"] = "empty"
                else:
                    source_health["rqdatac_econ"] = "invalid_payload"
            else:
                source_health["rqdatac_econ"] = "unavailable"
        except Exception as exc:
            source_health["rqdatac_econ"] = f"failed:{exc.__class__.__name__}"
            logger.info("[MacroAgent] China macro fetch failed: %s", exc)
        ...
        try:
            if hasattr(self.tools, "search"):
                cross_check_text = str(
                    self.tools.search("中国 最新 CPI PPI PMI M2 LPR 社融 宏观")
                    or ""
                )
        ...
        merged = self._merge_indicator_sources(
            primary_source="rqdatac_econ",
            primary=china_metrics,
        ...
        elif source_health.get("rqdatac_econ") == "ok":
            status = "success"
        else:
            status = "fallback"
        ...
        payload: Dict[str, Any] = {
            "status": status,
            "source": "rqdatac_econ" if source_health.get("rqdatac_econ") == "ok" else "search",
        ...
        for key in self._INDICATORS:
            ...
            if value is not None:
                payload[f"{key}_formatted"] = self._format_percentage_value(value, digits=2 if key in ("lpr", "bond_10y", "shibor") else 1)
        ...
        if china_payload.get("indicators"):
            payload["china_macro_raw"] = china_payload.get("indicators")
            ...
            for item in indicators:
                ...
                evidence.append(
                    EvidenceItem(
                        ...
                        confidence=0.9 if source == "rqdatac_econ" else 0.6,
                        meta={
