...
from backend.utils.kline import normalize_kline_payload
...
class TechnicalAgent(BaseFinancialAgent):
    ...
    def _get_tool_registry(self) -> dict:
        ...
        if kline_fn:
            registry["get_stock_historical_data"] = {
                ...
                "description": "获取A股日K线(ticker)，用于计算技术指标",
                "call_with": "ticker",
            }
        limit_fn = getattr(tools, "get_limit_board_info", None)
        if limit_fn:
            registry["get_limit_board_info"] = {
                "func": limit_fn,
                "description": "涨跌停价格与距离（主板±10%/科创±20%/北交±30%）",
                "call_with": "ticker",
            }
        return registry
    ...
    async def _initial_search(self, query: str, ticker: str) -> Dict[str, Any]:
        ...
        data = normalize_kline_payload(fetch(ticker, period="6mo", interval="1d"))
    ...
    async def _first_summary(self, data: Any) -> str:
        ...
        analysis = await self._llm_analyze(
            ...
            focus=(
                "A股技术面分析（注意 T+1、涨跌停板约束）：\n"
                "1. 趋势：MA20/50/200 排列与价格偏离\n"
                "2. 动量：RSI、MACD 是否共振或背离\n"
                "3. 关键位：支撑/压力；结合涨跌停距离评估上行/下行空间\n"
                "4. 形态：十字星、锤子线等中文常用形态（如有）\n"
                "5. 结论：1-2周与1-3月操作含义，勿暗示 T+0"
            ),
        )
