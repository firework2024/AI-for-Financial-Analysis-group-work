...
class FundamentalAgent(BaseFinancialAgent):
    ...
    _METRIC_DEFINITIONS: List[Dict[str, Any]] = [
        ...
        {"key": "operating_cash_flow", "label": "经营现金流", "table": "cashflow", "candidates": ["operating cash flow", "operating_cash_flow"]},
        {"key": "total_assets", "label": "总资产", "table": "balance", "candidates": ["total assets", "total_assets"]},
        {"key": "total_liabilities", "label": "总负债", "table": "balance", "candidates": ["total liabilities", "total_liabilities"]},
        {"key": "deducted_profit", "label": "扣非净利润", "table": "income", "candidates": ["deducted_profit", "扣非净利润"]},
        {"key": "goodwill", "label": "商誉", "table": "balance", "candidates": ["goodwill", "商誉"]},
        {"key": "accounts_receivable", "label": "应收账款", "table": "balance", "candidates": ["accounts_receivable", "应收账款"]},
    ]
    ...
    def _format_output(self, summary: str, raw_data: Any) -> AgentOutput:
        ...
        if isinstance(raw_data, dict):
            ...
            source = str((financials or {}).get("source") or "rqdatac")
            data_sources.append(source)
            _ticker = str(raw_data.get("ticker") or "").strip().upper()
            _fin_url = f"https://www.cninfo.com.cn/new/disclosure/stock?stockCode={_ticker.split('.')[0]}" if _ticker else None
            fallback_used = bool(
            ...
            if revision_signal in {"positive", "neutral", "negative"}:
                ...
                data_sources.append("rqdatac_consensus")
        ...
        return AgentOutput(
            ...
            data_sources=data_sources or ["rqdatac"],
