...
_EASTMONEY_USER_AGENT = os.getenv("EASTMONEY_USER_AGENT", "Mozilla/5.0 (AShareSight)")
...
def detect_market(ticker: str) -> str:
    ...
    if not symbol:
        return "UNKNOWN"
    if symbol.endswith((".SS", ".SZ", ".BJ", ".XSHG", ".XSHE", ".XBEX")):
        return "CN"
    if to_rqdatac_id(symbol):
        return "CN"
    if symbol.endswith(".HK"):
        return "HK"  # deprecated: AShareSight 仅服务 A 股，保留识别以兼容旧代码
    return "UNKNOWN"


def normalize_ticker(ticker: str) -> str:
    symbol = str(ticker or "").strip().upper()
    if symbol.endswith((".XSHG", ".XSHE", ".XBEX")):
        return from_rqdatac_id(symbol)
    rq = to_rqdatac_id(symbol)
    if rq:
        return from_rqdatac_id(rq)
    return symbol
...
def fetch_cn_hk_quote_metrics(ticker: str) -> dict[str, Any] | None:
    ...
    if market == "CN":
        try:
            from .rqdata_price import get_stock_price

            q = get_stock_price(ticker_norm)
            if isinstance(q, dict) and q.get("price") is not None:
                return {
                    "symbol": to_rqdatac_id(ticker_norm) or ticker_norm,
                    "market": "CN",
                    "name": q.get("name") or ticker_norm,
                    "last_price": q.get("price"),
                    "trailing_pe": None,
                    "forward_pe": None,
                    "price_to_book": None,
                    "source": q.get("source", "rqdatac"),
                }
        except Exception as exc:
            logger.info("[CNHK] rqdatac quote fallback: %s", exc)
