...
class _RagObservabilityStub:
    def ensure_schema(self) -> bool:
        return False


def get_rag_observability_store() -> _RagObservabilityStub:
    return _RagObservabilityStub()
...
def _clear_thread_rag_artifacts(thread_id: str) -> dict[str, int]:
    del thread_id
    return {"rag_collections": 0, "rag_runs": 0}
...
async def lifespan(app: FastAPI):
    ...
    try:
        from backend.tools.rqdata_client import ensure_rqdatac

        if ensure_rqdatac():
            logger.info("[RQData] connection ready")
        else:
            logger.warning("[RQData] not initialized; Eastmoney/search fallbacks will be used")
    except Exception as exc:
        logger.warning("[RQData] init check failed: %s", exc)
