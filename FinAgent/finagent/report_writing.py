...
def annual_director_structure_guide() -> str:
    return (
        ...
        "6. **核心矛盾汇总** — Markdown 表格，列矛盾维度与具体表现（可含 MD&A 与报表不一致处）。\n"
...
def section_writing_guide(section_name: str) -> str:
    ...
    hints: dict[str, str] = {
        ...
        "经营质量分析": (
            "按投资总监式经营质量报告写作：**核心经营表现概述**、**业务/收入结构变化**、"
            "**利润与费用驱动**、**现金流质量**、**营运资本与偿债**、**同行横向坐标**、"
            "**核心矛盾汇总**、**关注事项与数据局限**、**总结**；"
            "若有 mda_crosswalk 则在叙述中对照 MD&A 表述，不写独立勾稽章节；"
            "有 pit 或年报数据时必须输出多年对比表；禁止写 PE/PB/PS、股息率和估值判断。"
        ),
        "资金与交易结构": (
