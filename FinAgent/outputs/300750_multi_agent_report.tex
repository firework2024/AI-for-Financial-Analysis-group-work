...
截至2026年5月29日，宁德时代（300750.XSHE）收盘价424.0元，处于MA20（426.21元）下方0.52\%，但高于MA60（408.39元）3.82\%，中期趋势仍偏多。近60日累计涨幅达25.16\%，但近20日回调4.72\%，RSI（14日）为34.67，已进入偏弱区间，显示短期动能衰减。如图1（price\_volume双轴图）所示，价格在2026年5月6日触及区间高点465.88元后持续回落，与成交量同步萎缩，形成阶段性顶部。
...
**数据局限**
\begin{itemize}
\item 未提供融资融券余额日频数据，无法分析杠杆资金与股价的日内联动。
\item 未提供分时成交明细，无法判断大单资金流向与主力行为。
\item 未提供期权或期货数据，无法评估衍生品市场对现货价格的预期。
\end{itemize}
...
**数据局限**
\begin{itemize}
\item 未提供分季度财务数据，无法评估2026年Q1盈利趋势。
\item 未提供自由现金流具体数值，无法计算资本开支对现金流的影响。
\item 未提供行业可比公司估值数据，无法进行横向对比。
\item 未提供债务到期结构及利息覆盖倍数，无法深入评估偿债风险。
\end{itemize}
...
\begin{itemize}
\item 数据局限：JSON中未提供融券余额数据，无法绘制双轴图（融资余额左轴/融券余额右轴）；图表margin\_balances仅显示融资余额，未包含融券信息。
\end{itemize}
...
\begin{itemize}
\item 数据局限：JSON中未提供股东户数、机构持股比例、大股东增减持等股本结构数据。
\end{itemize}
...
**短端利率**
\begin{itemize}
\item 最新Shibor（2026-05-29）为1.50\%，较20日前（2026-05-09）的1.55\%下行5bp，反映短期流动性边际宽松。短端利率下行有助于降低宁德时代（300750.XSHE）短期融资成本，但其资产负债率62.32\%中长债占比更高，实际利好有限。
\end{itemize}
...
**与宁德时代估值的逻辑联系**
\begin{itemize}
\item **股息率利差**：宁德时代最新股息率（TTM）为1.88\%，低于10Y国债收益率2.10\%，差值约22bp。这意味着当前股息回报相对无风险利率缺乏吸引力，若长端利率继续上行，可能压制估值。宁德时代PE（TTM）为24.84倍，DCF折现率中无风险利率每上升10bp，理论估值约下降0.8\%-1.2\%（基于永续增长模型估算）。
\item **期限利差走阔**：10Y-1Y利差从0.45\%走阔至0.50\%，反映市场对长端利率预期偏稳。
\end{itemize}
...
**数据局限**
\begin{itemize}
\item 缺乏行业可比公司的估值与财务数据，无法进行横向对比。
\item 无宏观利率、汇率或大宗商品价格数据，无法评估外部环境变化对成本与需求的影响。
\item 无机构盈利预测或分析师评级数据，无法评估市场一致预期。
\item 无股东结构或机构持仓变化数据，无法判断资金流向。
\item 无分业务板块的详细财务数据，无法评估各业务线的风险贡献。
\end{itemize}
