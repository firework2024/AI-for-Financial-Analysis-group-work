...
FinAgent 是一个以 A 股年报为核心的本地命令行工作流。它会自动获取新浪财经年报纯文本，提取最新年报中的 MD&A 文本，从米筐 RQData 获取最近三年年报口径财务数据，并生成两层分析结果：
...
1. 输入 A 股股票代码和查询截止日期。
2. 从新浪财经查询正式年报纯文本，自动过滤摘要、英文版、更正、修订、补充等版本。
3. 保存最新年报纯文本到 `annual_reports/`。
4. 从纯文本中定位”管理层讨论与分析 / 经营情况讨论与分析”等 MD&A 正文（无需 PDF 解析）。
