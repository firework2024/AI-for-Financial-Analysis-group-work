...
const getInitialLayout = (): LayoutMode => {
  ...
  const stored = window.localStorage.getItem('asharesight-layout');
  return stored === 'full' || stored === 'centered' ? (stored as LayoutMode) : 'centered';
};

const getInitialTheme = (): Theme => {
  if (typeof window === 'undefined') return 'dark';
  const stored = window.localStorage.getItem('asharesight-theme');
...
const getInitialSubscriptionEmail = (): string => {
  ...
  return window.localStorage.getItem('asharesight-subscription-email') || '';
};

const getInitialEntryMode = (): EntryMode => {
  if (typeof window === 'undefined') return 'pending';
  const raw = window.localStorage.getItem('asharesight-entry-mode');
...
const getInitialSessionId = (): string | null => {
  ...
  const raw = window.localStorage.getItem('asharesight-session-id');
  if (!raw) {
    const generated = buildAnonymousSessionId();
    window.localStorage.setItem('asharesight-session-id', generated);
    return generated;
  }
...
const getInitialTraceRawEnabled = (): boolean => {
  ...
  const raw = window.localStorage.getItem('asharesight-trace-raw-enabled');
  if (raw === null) return false;
  return raw === 'true';
};
const getInitialTraceRawShowRawJson = (): boolean => {
  if (typeof window === 'undefined') return false;
  const raw = window.localStorage.getItem('asharesight-trace-raw-show-json');
  if (raw === null) return false;
  return raw === 'true';
};
const getInitialTraceViewMode = (): TraceViewMode => {
  if (typeof window === 'undefined') return 'user';
  const raw = window.localStorage.getItem('asharesight-trace-view-mode');
...
const getInitialPortfolioPositions = (): PortfolioPositions => {
  if (typeof window === 'undefined') return {};
  try {
    const raw = window.localStorage.getItem('asharesight-portfolio-positions');
    if (!raw) return {};
...
const persistPortfolioPositions = (positions: PortfolioPositions) => {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem('asharesight-portfolio-positions', JSON.stringify(positions));
};
...
const WELCOME_MESSAGE: Message = {
  ...
  role: 'assistant',
  content:
    '您好，我是 AShareSight 金融助手。直接输入股票代码或问题（例如：600519.XSHG 股价走势、贵州茅台最新新闻），我会用实时数据和图表帮你分析。',
  timestamp: Date.now(),
};

const MESSAGES_STORAGE_PREFIX = 'asharesight-messages:';
const CONVERSATIONS_STORAGE_KEY = 'asharesight-conversations';
...
export const useStore = create<AppState>((set) => ({
  ...
  startNewChat: () =>
    set((state) => {
      ...
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-session-id', nextSessionId);
      }
  ...
  setTheme: (theme) => {
    ...
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('asharesight-theme', theme);
    }
  },
  ...
  setLayoutMode: (mode) =>
    set(() => {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-layout', mode);
      }
  ...
  setSubscriptionEmail: (email) =>
    set(() => {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-subscription-email', email);
      }
  ...
  setEntryMode: (mode) =>
    set(() => {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-entry-mode', mode);
      }
  ...
  setSessionId: (sessionId) =>
    set((state) => {
      ...
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-session-id', normalized);
      }
  ...
  setTraceRawEnabled: (enabled) =>
    set(() => {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-trace-raw-enabled', String(Boolean(enabled)));
      }
  ...
  setTraceViewMode: (mode) =>
    set(() => {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-trace-view-mode', mode);
      }
  ...
  setTraceRawShowRawJson: (show) =>
    set(() => {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('asharesight-trace-raw-show-json', String(Boolean(show)));
      }
