    ...
    <div className="space-y-4">
      ...
      <Card className="px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="text-xs px-2.5 py-1.5 rounded-lg border border-ash-border hover:border-ash-primary/50 text-ash-text-secondary hover:text-ash-primary transition-colors"
          >
            回到上游（仪表盘）
          </button>
        </div>
      ...
      <Card className="p-4 space-y-3" data-testid="workbench-report-view">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <div className="text-sm font-semibold text-ash-text">工作台报告视图</div>
            <div className="text-2xs text-ash-muted mt-0.5">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              className="px-3 py-1.5 rounded-lg border border-ash-border text-xs text-ash-text hover:bg-ash-border/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <button
              type="button"
              className="px-3 py-1.5 rounded-lg border border-ash-warning/40 bg-ash-warning/10 text-xs text-ash-warning hover:bg-ash-warning/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {loadingSelectedReport && (
          <div className="rounded-lg border border-ash-border bg-ash-card/60 px-4 py-5 text-sm text-ash-muted" data-testid="workbench-report-loading">
            正在加载报告详情...
          </div>
        )}

        {!loadingSelectedReport && selectedReportError && (
          <div className="rounded-lg border border-ash-danger/40 bg-ash-danger/10 px-4 py-3 text-sm text-ash-danger" data-testid="workbench-report-error">
            {selectedReportError}
          </div>
        )}

        {!loadingSelectedReport && !selectedReportError && !selectedReport && (
          <div className="rounded-lg border border-ash-border bg-ash-card/60 px-4 py-5 text-sm text-ash-muted" data-testid="workbench-report-empty">
            请先在右侧报告时间线中选择一份报告。
          </div>
        )}
        ...
        {!loadingSelectedReport && selectedReport && (
          <>
            {hasTickerMismatch && (
              <div
                className="rounded-xl border border-ash-danger/50 bg-ash-danger/10 px-4 py-3"
                data-testid="workbench-report-ticker-mismatch"
              >
                <div className="text-sm font-semibold text-ash-danger">
                  ⚠ 报告标的不匹配，结论区已禁用
                </div>
                <div className="mt-1 text-xs text-ash-text/90">
            ...
            {showQualityBlockedBanner && (
              ...
              >
                ...
                <div className="mt-1 text-xs text-ash-text/85">
                  以下硬阈值未满足，请先补齐证据后再复用该报告。
                </div>
                {blockedReasons.length > 0 && (
                  <div className="mt-2 space-y-1 text-xs text-ash-text/85">
                    {blockedReasons.slice(0, 5).map((item, idx) => (
                      <div key={`${item.code}-${idx}`}>
                        <div>• {item.message || item.code}</div>
                        <div className="text-2xs text-ash-text/65">
                          code={item.code}
            ...
            {qualityMissing.length > 0 && (
              <div
                className="rounded-xl border border-ash-warning/40 bg-ash-warning/10 px-4 py-3"
                data-testid="workbench-report-quality-gap"
              >
                <div className="text-sm font-semibold text-ash-warning">证据不足（质量门槛未满足）</div>
                <div className="mt-1 text-xs text-ash-text/80">
                ...
                <div className="mt-2 space-y-2">
                  {qualityMissing.slice(0, 6).map((item, idx) => (
                    <div key={`${idx}-${item}`} className="flex items-start gap-2 text-xs">
                      ...
                      <span className="flex-1 text-ash-text/85">{item}</span>
                      <button
                        type="button"
            ...
            {verifierClaims.length > 0 && (
              <div
                className="rounded-xl border border-ash-danger/40 bg-ash-danger/10 px-4 py-3"
            ...
            {hasTickerMismatch ? (
              <div
                className="rounded-xl border border-ash-border bg-ash-card/60 px-4 py-6 text-sm text-ash-muted"
