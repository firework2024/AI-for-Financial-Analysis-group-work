...
export function Dashboard({ initialSymbol, onBackToChat, onSymbolChange, onGoWorkbench }: DashboardProps) {
  ...
  if (!currentSymbol) {
    return (
      <div className="flex-1 min-h-0 flex overflow-hidden max-lg:flex-col">
        <aside className="w-[220px] shrink-0 border-r border-ash-border bg-ash-card flex flex-col max-lg:w-full max-lg:h-[220px] max-lg:border-r-0 max-lg:border-b">
          <Watchlist activeSymbol="" onSymbolSelect={handleSymbolChange} />
        </aside>
        <main className="flex-1 min-w-0 min-h-0 flex flex-col items-center justify-center bg-ash-bg">
          <div className="text-center max-w-md px-6">
            <div className="text-4xl mb-4">📳</div>
            <h2 className="text-lg font-semibold text-ash-text mb-2">选择一只股票开始分析</h2>
  ...
  return (
    <div className={[
      ...
      isTerminalStyle ? 'bg-[#0a0e17] text-slate-100' : 'bg-ash-bg text-ash-text',
    ].join(' ')}>
      {isTerminalStyle && (
        <div className="h-9 shrink-0 border-b border-[#1e2a3a] bg-[#111827] px-4 flex items-center justify-between text-[11px] font-mono">
          <div className="flex items-center gap-5 text-slate-400">
            <span className="text-[#ff8c00] font-semibold tracking-wide">ASHARESIGHT</span>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden max-lg:flex-col">
        <aside
          className={[
            ...
            isTerminalStyle ? 'border-[#1e2a3a] bg-[#111827]' : 'border-ash-border bg-ash-card',
          ].join(' ')}
        >
        ...
        <main className={[
          ...
          isTerminalStyle ? 'bg-[#0a0e17]' : 'bg-ash-bg',
        ].join(' ')}>
          <header
            className={[
              'h-[52px] border-b flex items-center justify-between px-5 shrink-0 max-lg:px-3',
              isTerminalStyle ? 'bg-[#111827] border-[#1e2a3a]' : 'bg-ash-card border-ash-border',
            ].join(' ')}
          >
            <div className="flex items-center gap-3 min-w-0">
                  ...
                  <button
                    ...
                    className={[
                      ...
                      isTerminalStyle
                        ...
                        : 'border-ash-border bg-ash-bg text-ash-text-secondary hover:bg-ash-hover',
                    ].join(' ')}
                  >
                    返回对话
                  </button>
                    ...
                    <button
                      ...
                      className={[
                        ...
                        isTerminalStyle
                          ...
                          : 'border-ash-primary/40 bg-ash-primary/10 text-ash-primary hover:bg-ash-primary/20',
                      ].join(' ')}
                    >
                      去工作台
                    </button>
              ...
              {isLoading && <span className="text-xs text-ash-muted animate-pulse shrink-0">加载中...</span>}
            </div>
          ...
          {error && (
            <div className="mx-5 mt-4 p-2 bg-ash-danger/10 border border-ash-danger/30 rounded-lg text-ash-danger text-sm flex items-center gap-2 shrink-0 max-lg:mx-3">
              <span className="font-medium">加载失败:</span>
              <span className="truncate flex-1">{error}</span>
              <button
                type="button"
                onClick={handleRefresh}
                className="text-ash-danger font-medium underline hover:no-underline whitespace-nowrap"
              >
                重试
              </button>
            </div>
          ...
          {isTerminalStyle && (
            <div className="h-8 shrink-0 border-t border-[#1e2a3a] bg-[#111827] overflow-hidden flex items-center">
              <div className="flex w-max items-center gap-8 px-4 text-[11px] font-mono text-slate-300" style={{ animation: 'asharesight-marquee 36s linear infinite' }}>
