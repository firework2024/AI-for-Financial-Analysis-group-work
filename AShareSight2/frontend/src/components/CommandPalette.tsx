...
export const CommandPalette: FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const actions: CommandAction[] = useMemo(
    () => [
      ...
      },
      {
        ...
        execute: () => {
          window.dispatchEvent(new CustomEvent('asharesight:open-settings'));
          onClose();
        },
      },
      {
  ...
  return (
    <Dialog
      ...
      panelClassName="bg-ash-card border border-ash-border rounded-xl w-full max-w-lg shadow-2xl overflow-hidden"
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-ash-border">
        <Search size={18} className="text-ash-muted shrink-0" />
        <input
          ref={inputRef}
          type="text"
          value={query}
          ...
          placeholder="搜索命令..."
          aria-label="搜索命令"
          className="flex-1 bg-transparent text-ash-text text-sm placeholder:text-ash-muted outline-none border-none"
        />
        <kbd className="hidden sm:inline-flex items-center gap-1 px-1.5 py-0.5 rounded border border-ash-border text-[10px] text-ash-muted font-mono">
          ESC
        </kbd>
      </div>
      ...
      <div ref={listRef} className="max-h-64 overflow-y-auto py-2" role="listbox">
        {filteredActions.length === 0 ? (
          <div className="px-4 py-6 text-center text-sm text-ash-muted">没有匹配的命令</div>
        ) : (
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors rounded-lg mx-auto cursor-pointer ${
                  isActive ? 'bg-ash-primary/10 text-ash-primary' : 'text-ash-text hover:bg-ash-hover'
                }`}
              >
                <Icon size={16} className="shrink-0" />
                <span className="flex-1 text-left">{action.label}</span>
                {action.shortcut && (
                  <kbd className="text-[10px] text-ash-muted font-mono border border-ash-border px-1.5 py-0.5 rounded">
                    {action.shortcut}
                  </kbd>
                )}
              </button>
      ...
      <div className="flex items-center gap-4 px-4 py-2 border-t border-ash-border text-[10px] text-ash-muted">
        <span className="flex items-center gap-1">
          <kbd className="px-1 py-0.5 rounded border border-ash-border font-mono">&uarr;&darr;</kbd>
          导航
        </span>
        <span className="flex items-center gap-1">
          <kbd className="px-1 py-0.5 rounded border border-ash-border font-mono">Enter</kbd>
          选择
        </span>
        <span className="flex items-center gap-1">
          <kbd className="px-1 py-0.5 rounded border border-ash-border font-mono">{modKey}+K</kbd>
          切换
        </span>
      </div>
    </Dialog>
