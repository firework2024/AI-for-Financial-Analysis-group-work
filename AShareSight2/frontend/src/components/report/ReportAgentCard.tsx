...
const JsonViewer: React.FC<{ label: string; value: any }> = ({ label, value }) => {
  ...
  return (
    <div className="rounded-lg border border-ash-border overflow-hidden">
      <div className="px-3 py-2 bg-ash-bg-secondary flex items-center justify-between">
        <span className="text-xs font-medium text-ash-text">{label}</span>
        <button
          type="button"
          className="inline-flex items-center gap-1 text-2xs text-ash-text-secondary hover:text-ash-text"
          onClick={async () => {
      ...
      <pre className="p-3 text-[11px] leading-relaxed overflow-auto max-h-[320px] bg-ash-card text-ash-text whitespace-pre-wrap break-all">
        {content}
      </pre>
    </div>
  );
...
const EvidenceExcerptList: React.FC<{ evidence: any[] }> = ({ evidence }) => {
  if (!Array.isArray(evidence) || evidence.length === 0) {
    return (
      <div className="rounded-lg border border-ash-border p-3 text-xs text-ash-text-secondary">
        暂无证据摘录
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-ash-border bg-ash-bg-secondary p-3 space-y-3">
      {rows.map((item, index) => {
        ...
        return (
          <div key={`${sourceId}-${index}`} className="text-xs text-ash-text">
            ...
            {url && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-1 inline-block text-ash-primary hover:underline break-all"
              >
                {url}
              </a>
            )}
...
export const ReportAgentCard: React.FC<ReportAgentCardProps> = ({
  ...
  if (agentDetailSections.length > 0) {
    return (
      <details className="group rounded-xl border border-ash-border bg-ash-card overflow-hidden">
        <summary className="px-5 py-3 cursor-pointer hover:bg-ash-hover transition-colors flex items-center gap-2">
          <ChevronDown size={16} className="text-ash-muted group-open:rotate-180 transition-transform" />
          <span className="text-sm font-semibold text-ash-text">Agent 分析详情</span>
          <span className="text-2xs text-ash-muted ml-auto">{agentDetailSections.length} 个数据源</span>
        </summary>
          ...
          {agentDetailSections.map((section) => {
            ...
            return (
              <div
                ...
                className="border border-ash-border rounded-xl overflow-hidden bg-ash-card shadow-sm"
              >
                <button
                  type="button"
                  onClick={() => onToggleSection(section.order)}
                  className="w-full flex items-center justify-between px-4 py-3 bg-ash-bg-secondary hover:bg-ash-hover transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <span className="h-6 w-6 rounded-full bg-ash-primary/15 text-ash-primary flex items-center justify-center text-xs font-bold">
                      {section.order}
                    </span>
                    <div className="text-left">
                      <div className="text-sm font-semibold text-ash-text">{section.title}</div>
                      <div className="text-2xs text-ash-text-secondary mt-0.5">
                  ...
                  <ChevronDown
                    size={16}
                    className={`text-ash-muted transition-transform ${isOpen ? 'rotate-180' : ''}`}
                  />
                </button>

                {isOpen && (
                  <div className="p-4 space-y-3">
                    <div className="rounded-lg border border-ash-border p-3 bg-ash-bg-secondary">
                      <div className="text-2xs text-ash-text-secondary mb-1">Summary</div>
                      <div className="text-xs leading-relaxed text-ash-text whitespace-pre-wrap">
                        {summary}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 text-2xs">
                      <button
                        ...
                        className={`underline-offset-2 hover:underline ${
                          evidenceCount === 0
                            ? 'text-ash-muted cursor-not-allowed'
                            : 'text-ash-text-secondary'
                        }`}
                      >
                      ...
                      {isDeveloper && (
                        <button
                          ...
                          className="text-ash-primary underline-offset-2 hover:underline"
                        >
                    ...
                    {isDeveloper && isDebugOpen && (
                      <div className="space-y-3">
                        ...
                        <details className="rounded-lg border border-ash-border overflow-hidden">
                          <summary className="px-3 py-2 cursor-pointer bg-ash-bg-secondary text-xs font-medium text-ash-text">
                            原始输出（采集/trace）
                          </summary>
                          <div className="p-3 space-y-3 bg-ash-card border-t border-ash-border">
  ...
  return (
    <div className="rounded-xl border border-ash-border bg-ash-card overflow-hidden">
      <div className="px-5 py-3 text-sm font-semibold text-ash-text border-b border-ash-border">
        Agent 分析详情
      </div>
      <div className="px-5 py-4 text-xs text-ash-muted">暂无 agent 分析</div>
    </div>
  );
};
...
export const ReportEvidencePoolSection: React.FC<ReportEvidencePoolProps> = ({
  ...
  return (
    <details
      className="group rounded-xl border border-ash-border bg-ash-card overflow-hidden"
      open
    >
      <summary className="px-5 py-3 cursor-pointer hover:bg-ash-hover transition-colors flex items-center gap-2">
        <ChevronDown size={16} className="text-ash-muted group-open:rotate-180 transition-transform" />
        <span className="text-sm font-semibold text-ash-text">证据池</span>
        <span className="text-2xs text-ash-muted ml-auto">{citations.length} 条来源</span>
      </summary>
