    ...
    <div className="flex flex-col h-full">
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto space-y-3 p-2">
        {recentMessages.length <= 1 ? (
          <div className="text-center text-ash-muted text-xs py-8">
            <p>👋 Hi! 有什么可以帮你的？</p>
            ...
            >
              <div
                className={`max-w-[85%] px-3 py-2 rounded-xl text-xs ${
                  msg.role === 'user'
                    ? 'bg-ash-primary text-white'
                    : 'bg-ash-bg border border-ash-border text-ash-text'
                }`}
              >
                {msg.isLoading ? (
                  <span className="flex items-center gap-1.5">
                    <Loader2 size={12} className="animate-spin" />
                    <span className="text-ash-muted">思考中...</span>
                  </span>
                ) : msg.role === 'user' ? (
                  // 用户消息：纯文本
                  ...
                  msg.report ? (
                    ...
                    <div className="prose prose-sm prose-invert max-w-none text-ash-text prose-p:my-1 prose-headings:my-1.5 prose-ul:my-1 prose-ol:my-1 prose-li:my-0.5 prose-pre:my-1 prose-code:text-ash-primary prose-code:bg-ash-bg/50 prose-code:px-1 prose-code:rounded">
      ...
      {/* Input Area */}
      <div className="border-t border-ash-border p-2 space-y-1.5">
        {/* Context Pills - Symbol 和 Selection */}
        <div className="flex flex-wrap items-center gap-1.5">
          {/* Symbol Context Pill */}
          {showContextPill && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-ash-primary/10 text-ash-primary text-2xs font-medium">
              <span>📌 {currentSymbol}</span>
              <button
                onClick={() => setContextEnabled(false)}
                className="ml-0.5 hover:bg-ash-primary/20 rounded-full p-0.5 transition-colors"
                title="关闭上下文注入"
              >
                <X size={10} />
              </button>
          ...
          <button
            ...
            className={`px-1.5 py-0.5 rounded border text-2xs transition-colors ${
              outputMode === 'investment_report'
                ...
                : 'border-ash-border text-ash-text-secondary hover:border-amber-500/50'
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            disabled={isLoading}
            rows={1}
            className="flex-1 px-3 py-2 text-xs bg-ash-bg border border-ash-border rounded-lg text-ash-text placeholder:text-ash-muted focus:outline-none focus:border-ash-primary disabled:opacity-50 resize-none overflow-y-hidden max-h-[80px]"
          />
          {isLoading ? (
            <button
              data-testid="mini-chat-stop-btn"
              onClick={handleStop}
              className="p-2 bg-ash-danger text-white rounded-lg hover:opacity-90 transition-opacity"
              title="停止生成"
              aria-label="停止生成"
            >
            ...
            <button
              ...
              onClick={() => handleSend()}
              disabled={!input.trim()}
              className="p-2 bg-ash-primary text-white rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
