...
export const StockChart: React.FC = () => {
  ...
  if (!currentTicker) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-ash-muted space-y-4 animate-fade-in">
        <div className="p-4 bg-ash-panel rounded-full">
  ...
  if (loading) {
    return (
      <div className="h-full flex items-center justify-center text-ash-primary animate-pulse">
  ...
  const candlestickOption = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const changeColor = change >= 0 ? chartTheme.danger : chartTheme.success;
    ...
    series: [
      {
        ...
        itemStyle: {
          color: chartTheme.danger,        // 涨 (红)
          color0: chartTheme.success,       // 跌 (绿)
          borderColor: chartTheme.danger,
          borderColor0: chartTheme.success
        }
      }
    ]
  };
      ...
      formatter: (params: any) => {
        ...
        return `
          <div style="padding: 8px;">
            ...
            <div>涨跌: <span style="color: ${data.value >= 0 ? chartTheme.danger : chartTheme.success}">${sign}${data.value.toFixed(2)}%</span></div>
          </div>
        `;
      }
    },
  ...
  return (
    <div className="h-full w-full flex flex-col animate-slide-up">
      {/* 控制栏 */}
      <div className="flex items-center justify-between p-2 border-b border-ash-border bg-ash-panel">
        {/* 时间周期选择 */}
          ...
          {PERIOD_OPTIONS.map(opt => (
            <button
              ...
              className={`px-2 py-1 text-xs rounded transition-colors ${period === opt.value
                ...
                : 'bg-ash-bg text-ash-muted hover:bg-ash-border'
                }`}
            >
              {opt.label}
            </button>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`p-1.5 rounded transition-colors ${chartType === 'candlestick'
              ? 'bg-ash-primary text-white'
              : 'text-ash-muted hover:bg-ash-border'
              }`}
            title="K线图"
          >
            <Activity size={16} />
