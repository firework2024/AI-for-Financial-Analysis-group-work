    ...
    >
        ...
        >
          <div className={clsx(
            ...
            <div className={clsx(
              ...
              msg.role === 'user' ? "bg-ash-primary text-white" : "bg-ash-panel border border-ash-border text-ash-primary"
            )}>
            ...
            <div className={clsx(
              ...
              msg.role === 'user'
                ? "bg-ash-hover text-ash-text rounded-tr-sm"
                : "bg-ash-panel border border-ash-border text-ash-text rounded-tl-sm relative overflow-visible"
            )}>
              {msg.role === 'user' ? (
                msg.content
              ) : (
                <>
                  ...
                  {msg.evidence_pool && msg.evidence_pool.length > 0 && (
                    <div className="mt-3 rounded-lg border border-ash-border/60 bg-ash-bg/40 px-3 py-2">
                      <div className="text-[11px] text-ash-muted mb-2">Evidence ({msg.evidence_pool.length})</div>
                      <div className="flex flex-wrap gap-2">
                        {msg.evidence_pool.map((ev: EvidenceItem, idx: number) => {
                          ...
                          return (
                            <span key={`ev-${idx}`} className="px-2 py-1 rounded-full border border-ash-border/70 bg-ash-panel text-[11px] text-ash-text">
                              {label}
                            </span>
                          );
                        })}
                  ...
                  {msg.data_origin && (
                    <div className="mt-2 text-[11px] text-ash-muted flex items-center gap-2">
                      <span className="px-2 py-0.5 rounded-full border border-ash-border/60 bg-ash-bg/60">
                        来源: {msg.data_origin} {msg.fallback_used ? '(兜底)' : ''}
                      </span>
                      {msg.as_of && <span className="px-2 py-0.5 rounded-full border border-ash-border/60 bg-ash-bg/60">截至: {msg.as_of}</span>}
                      {msg.tried_sources && msg.tried_sources.length > 0 && (
                        <span className="text-2xs text-ash-muted/70">
      ...
      {showExecutionBanner && (
        <div className="flex w-full justify-start animate-fade-in">
          <div className="ml-12 rounded-xl border border-ash-border/60 bg-ash-panel/60 px-3 py-2 shadow-sm min-w-[280px] max-w-[420px]">
            <div className="flex items-center gap-2">
              <div className="flex space-x-1">
                <div className="w-2 h-2 bg-ash-muted rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                <div className="w-2 h-2 bg-ash-muted rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                <div className="w-2 h-2 bg-ash-muted rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
              </div>
            ...
            <div className="mt-2">
              <div className="h-1.5 rounded-full bg-ash-border/70 overflow-hidden">
                <div
                  className="h-full rounded-full bg-ash-primary transition-all duration-300"
                  style={{ width: `${Math.max(0, Math.min(100, executionProgress ?? 0))}%` }}
                />
              </div>
              <div className="mt-1 flex items-center justify-between text-2xs text-ash-muted">
...
const SourceLink: React.FC<{ href: string; label: React.ReactNode }> = ({ href, label }) => {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-ash-border/70 bg-ash-bg hover:border-ash-primary/80 hover:text-ash-primary transition text-ash-text no-underline"
      title={href}
    >
      {urlMeta.domain ? <Link2 size={14} /> : <ExternalLink size={14} />}
      <span className="truncate max-w-[160px]">{displayText}</span>
      {urlMeta.domain && (
        <span className="text-2xs text-ash-muted/70">({urlMeta.domain})</span>
      )}
    </a>
  );
};
...
const MessageActions: React.FC<{
  ...
  return (
    <div className="absolute bottom-0 right-2 translate-y-full flex items-center gap-3 text-ash-muted pointer-events-auto">
      <button
        className="p-1 rounded hover:text-ash-text"
        title="复制"
        onClick={handleCopy}
      >
        <Copy size={14} />
...
const LoadingDots: React.FC = () => (
  <div className="flex space-x-2">
    <span className="w-2 h-2 rounded-full bg-ash-muted animate-bounce" style={{ animationDelay: '0ms' }} />
    <span className="w-2 h-2 rounded-full bg-ash-muted animate-bounce" style={{ animationDelay: '150ms' }} />
    <span className="w-2 h-2 rounded-full bg-ash-muted animate-bounce" style={{ animationDelay: '300ms' }} />
  </div>
);
