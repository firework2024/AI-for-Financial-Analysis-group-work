...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Dialog
      ...
      panelClassName="bg-ash-panel border border-ash-border rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl"
    >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-ash-border sticky top-0 bg-ash-panel z-10">
          <div className="flex items-center gap-2">
            <Settings size={20} className="text-ash-primary" />
            <h2 id="settings-modal-title" className="text-lg font-semibold text-ash-text">设置</h2>
          </div>
          ...
          <Button
            ...
            className="p-1"
          >
            <X size={20} className="text-ash-muted" />
          </Button>
        </div>
        ...
        <div className="p-6 space-y-6">
          {/* LLM 配置 — 使用共享 Card 组件 */}
          <Card className="p-4 bg-ash-bg/40">
            <h3 className="text-sm font-medium text-ash-text mb-3">LLM 配置（可选）</h3>
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              <div>
                <label className="block text-xs text-ash-muted mb-1">提供商</label>
                <select
                  value={config.llm_provider || ''}
                  onChange={(e) => handleChange('llm_provider', e.target.value)}
                  className="w-full bg-ash-bg border border-ash-border rounded px-3 py-2 text-ash-text text-sm focus:border-ash-primary outline-none"
                >
              ...
              <div className="md:col-span-2">
                <label className="block text-xs text-ash-muted mb-1">API Key</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-ash-bg border border-ash-border rounded px-3 py-2 pr-10 text-ash-text text-sm focus:border-ash-primary outline-none"
                  />
                  <button
                    type="button"
                    aria-label={showLegacyApiKey ? '隐藏 API Key' : '显示 API Key'}
                    onClick={() => setShowLegacyApiKey((v) => !v)}
                    className="absolute inset-y-0 right-2 flex items-center text-ash-muted hover:text-ash-primary"
                  >
          ...
          <Card className="p-4 bg-ash-bg/40" data-testid="settings-endpoints-section">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium text-ash-text">LLM Endpoints（轮换池）</h3>
              ...
              <div className="space-y-3">
                {(config.llm_endpoints || []).map((endpoint, index) => (
                  <div
                    ...
                    className="border border-ash-border rounded p-3 bg-ash-panel/40 space-y-3"
                  >
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-ash-muted">Endpoint #{index + 1}</p>
                    ...
                    <div className="grid md:grid-cols-2 gap-3 text-sm">
                      ...
                      <div>
                        <label className="block text-xs text-ash-muted mb-1">Provider</label>
                        <select
                    ...
                    <label className="inline-flex items-center gap-2 text-xs text-ash-muted cursor-pointer">
                      <input
                        type="checkbox"
            ...
            <p className="text-xs text-ash-muted mt-3">
          ...
          {/* 界面布局 — 使用共享 Card 组件 */}
          <Card className="p-4 bg-ash-bg/40">
            <h3 className="text-sm font-medium text-ash-text mb-3">界面布局</h3>
            <div className="flex gap-4 text-sm">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="accent-ash-primary"
                />
                <span className="text-ash-text">居中布局</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="radio"
                  ...
                  className="accent-ash-primary"
                />
                <span className="text-ash-text">铺满宽度</span>
              </label>
            </div>
          </Card>
          ...
          {/* 外观 — 使用共享 Card 组件 */}
          <Card className="p-4 bg-ash-bg/40">
            <h3 className="text-sm font-medium text-ash-text mb-3">外观主题</h3>
            <div className="flex gap-4">
              <button
                onClick={() => setTheme('light')}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${theme === 'light'
                  ? 'border-ash-primary bg-ash-primary/10 text-ash-primary'
                  : 'border-ash-border text-ash-text hover:border-ash-primary/50'
                  }`}
              >
                <Sun size={16} /> 浅色
              </button>
          ...
          <Card className="p-4 bg-ash-bg/40">
            <h3 className="text-sm font-medium text-ash-text mb-3">Trace 可见性</h3>
            <div className="grid md:grid-cols-2 gap-3 text-sm">
              <button
                type="button"
            ...
            <div className="mt-3">
              <div className="text-xs text-ash-muted mb-2">Trace 展示层级</div>
          ...
          <Card className="p-4 bg-ash-bg/40">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium text-ash-text flex items-center gap-2">
                <Activity size={16} className="text-ash-primary" />
                系统状态 & 诊断
              </h3>
              ...
              <Button
                ...
                className="text-xs text-ash-muted hover:text-ash-primary"
              >
            ...
            <div className="grid md:grid-cols-2 gap-4 text-xs">
              {/* Orchestrator */}
              <div className="border border-ash-border rounded p-3 bg-ash-panel/60">
                <div className="font-medium text-ash-text mb-2 flex items-center gap-1">
                ...
                {orchestratorStats?.data ? (
                  <div className="space-y-1 text-ash-muted">
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          {sources.length > 0 && (
                            <div className="pt-1 border-t border-ash-border mt-1">
                  ...
                  <div className="text-ash-muted">无数据</div>
                )}
              </div>
            </div>
          </Card>
          ...
          {/* 提示 */}
          <div className="p-3 bg-ash-bg border border-ash-border rounded text-xs text-ash-muted">
        ...
        <div className="flex items-center justify-end gap-3 p-4 border-t border-ash-border sticky bottom-0 bg-ash-panel">
          <Button
            variant="ghost"
            size="md"
            data-testid="settings-cancel-btn"
            onClick={onClose}
            className="px-4 py-2 text-ash-muted hover:text-ash-text"
          >
            取消
          </Button>
          <Button
