...
const PANEL_WIDTH_STORAGE_KEY = 'asharesight_right_panel_width';
...
export function WorkspaceShell({
  ...
  useEffect(() => {
    ...
    window.addEventListener('asharesight:open-settings', handleOpenSettings as EventListener);
    return () => {
      window.removeEventListener('asharesight:open-settings', handleOpenSettings as EventListener);
    };
  }, []);
  ...
  return (
    <div className="flex h-screen w-screen bg-ash-bg text-ash-text font-mono overflow-hidden">
      {/* Mobile menu button */}
      {isMobile && (
        <button
          onClick={() => setIsSidebarOpen(true)}
          className="fixed top-3 left-3 z-50 p-2 rounded-lg bg-ash-card border border-ash-border text-ash-text hover:bg-ash-hover transition-colors lg:hidden"
          aria-label="打开导航菜单"
        >
          <Menu size={20} />
        </button>
