...
export function ChatWorkspace({
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-col h-full overflow-hidden relative">
      <header className="h-[60px] bg-ash-card border-b border-ash-border flex items-center justify-between px-6 shrink-0 max-lg:px-3">
        <div className="flex gap-4 text-xs text-ash-text font-medium overflow-x-auto scrollbar-hide">
          {marketQuotes.map((quote) => (
            <span key={quote.label} className="flex items-center gap-1 whitespace-nowrap">
              ...
              {quote.loading ? (
                ...
                <span className={quote.changePct >= 0 ? 'text-ash-success' : 'text-ash-danger'}>
                  {formatChangePct(quote.changePct)}
                </span>
              ) : quote.price !== undefined ? (
                <span className="text-ash-warning">${quote.price.toLocaleString()}</span>
              ) : (
                <span className="text-ash-muted">--</span>
              )}
            </span>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-ash-border bg-ash-bg hover:bg-ash-hover transition-colors text-xs font-medium text-ash-text"
            title="新建对话"
            aria-label="新建对话"
          >
            <Plus size={14} />
          ...
          <button
            ...
            className="p-2 rounded-lg border border-ash-border bg-ash-bg hover:bg-ash-hover transition-colors text-ash-text-secondary"
          >
            {theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}
          </button>
          <button
            type="button"
            className="px-3 py-1.5 rounded-lg border border-ash-border bg-ash-bg hover:bg-ash-hover transition-colors text-xs font-medium text-ash-text"
          >
            导出 PDF
          </button>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden p-5 gap-4 max-lg:flex-col max-lg:p-3">
        <aside
          ...
          className="w-64 shrink-0 bg-ash-card border border-ash-border rounded-2xl overflow-hidden flex flex-col min-h-0 max-lg:w-full max-lg:max-h-44"
        >
          <div className="h-11 px-3 border-b border-ash-border flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2 text-xs font-semibold text-ash-text">
              <MessageSquare size={14} className="text-ash-muted" />
              会话
            </div>
            <button
              type="button"
              onClick={startNewChat}
              className="p-1.5 rounded-md text-ash-muted hover:text-ash-text hover:bg-ash-hover transition-colors"
              title="新建对话"
              aria-label="新建对话"
            >
              <Plus size={14} />
              ...
              return (
                <div
                  ...
                  className={`group w-full min-h-[58px] rounded-lg border transition-colors flex items-stretch ${
                    active
                      ? 'bg-ash-primary/10 border-ash-primary/30 text-ash-text'
                      : 'border-transparent text-ash-text-secondary hover:bg-ash-hover hover:text-ash-text'
                  }`}
                ...
                >
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-2xs text-ash-muted">
                        <span>{formatConversationTime(conversation.updatedAt)}</span>
                        <span>{conversation.messageCount} 条</span>
                      </div>
                      {conversation.lastMessagePreview && (
                        <div className="mt-1 truncate text-2xs text-ash-muted">
                          {conversation.lastMessagePreview}
                        </div>
                      )}
                    </div>
                  ...
                  <button
                    type="button"
                    className="shrink-0 self-start mt-1.5 mr-1.5 opacity-0 group-hover:opacity-100 focus:opacity-100 p-1 rounded-md text-ash-muted hover:text-red-300 hover:bg-red-500/10 transition"
                    title="删除会话"
                    aria-label="删除会话"
        ...
        <div className="flex-1 min-w-0 min-h-0 flex flex-col gap-3 overflow-hidden">
          <div className="flex-1 bg-ash-card border border-ash-border rounded-2xl flex flex-col overflow-hidden shadow-sm min-h-0">
            <ChatList />
            ...
            {traceViewMode === 'dev' ? (
              ...
              execCollapsed ? (
                <button
                  ...
                  className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl border border-ash-border bg-ash-card text-xs text-ash-muted hover:bg-ash-hover transition-colors"
                >
