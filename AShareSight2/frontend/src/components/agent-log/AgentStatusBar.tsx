...
export const AgentStatusBar: React.FC<AgentStatusBarProps> = ({
  ...
  return (
    <div className="flex items-center justify-between px-2 py-1 bg-ash-bg border-t border-ash-border text-2xs text-ash-muted">
      <div className="flex items-center gap-3">
        ...
        {!showTokens && stats.typeCounts['token'] > 0 && (
          <span className="text-ash-muted/70">({stats.typeCounts['token']} tokens hidden)</span>
        )}
      </div>
      <div className="flex items-center gap-2">
        ...
        {Object.entries(agentStatuses)
          ...
          .map(([key]) => (
            <span key={key} className="px-1 py-0.5 rounded bg-ash-success/10 text-ash-success text-[9px]">
              {key}
            </span>
          ))
        }
