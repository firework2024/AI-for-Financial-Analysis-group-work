...
export const AgentLogPanel: React.FC = () => {
  ...
  if (!isConsoleOpen) {
    return (
      <button
        ...
        className="w-full p-2.5 bg-ash-card border border-ash-border rounded-xl hover:bg-ash-hover transition-colors flex items-center justify-between group"
      >
        <div className="flex items-center gap-2">
          <Terminal size={13} className="text-ash-primary" />
          <span className="text-[11px] font-mono font-bold text-ash-text-secondary uppercase tracking-wider">Console</span>
        </div>
        <div className="flex items-center gap-2">
          {runningAgents > 0 && (
          ...
          <span className="text-2xs text-ash-muted font-mono">{rawEvents.length} events</span>
          <ChevronDown size={12} className="text-ash-muted group-hover:text-ash-text" />
        </div>
      </button>
    );
  }
  ...
  const consoleContent = (
    <div
      className={`flex flex-col bg-ash-card overflow-hidden font-mono shadow-sm ${isMaximized ? 'h-full' : 'border border-ash-border rounded-xl'}`}
    ...
    >
      ...
      {/* Lens controls (A/B) */}
      <div className="flex items-center gap-2 px-2 py-1 bg-ash-bg border-b border-ash-border/50 overflow-x-auto scrollbar-hide">
        <button
          type="button"
          ...
          className={`px-2 py-0.5 rounded text-[10px] font-semibold whitespace-nowrap ${
            consoleLens === 'event_stream'
              ...
              : 'bg-ash-panel text-ash-muted border border-ash-border hover:text-ash-text'
          }`}
        ...
        <span className="text-[9px] text-ash-muted whitespace-nowrap">
          {consoleLens === 'event_stream'
            ? '聚焦关键执行事件'
      ...
      {consoleLens === 'agent_pipeline' && (
        <div className="flex items-center gap-1 px-2 py-1 bg-ash-panel border-b border-ash-border/50 overflow-x-auto scrollbar-hide">
          <button
            ...
            className="px-1.5 py-[1px] rounded text-[9px] border border-ash-border text-ash-muted hover:text-ash-text whitespace-nowrap"
          >
            全选
          </button>
          <button
            ...
            return (
              <button
                ...
                className={`px-1.5 py-[1px] rounded text-[9px] border whitespace-nowrap transition-colors ${
                  active
                    ? 'border-ash-primary/40 bg-ash-primary/10 text-ash-primary'
      ...
      <div className={`flex flex-1 min-h-0 ${isMaximized ? '' : 'max-h-[35vh]'}`}>
        ...
        <div
          ...
          className={`overflow-y-auto overflow-x-hidden scrollbar-thin ${
            selectedEvent ? 'w-1/2 border-r border-ash-border' : 'w-full'
          }`}
        >
          {filteredEvents.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-ash-muted">
              ...
              <p className="text-2xs text-ash-muted/70">
                {rawEvents.length === 0
  ...
  if (isMaximized) {
    return (
      <div className="fixed inset-0 z-[200] bg-ash-bg/95 backdrop-blur-sm flex flex-col">
        {consoleContent}
      </div>
    );
  }
