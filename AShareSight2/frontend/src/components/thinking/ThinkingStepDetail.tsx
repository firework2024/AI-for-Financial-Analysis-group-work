...
const formatDetailedResult = (result: any): React.ReactNode => {
  ...
  if (result.intent) {
    items.push(
      <div key="intent" className="flex items-center gap-2 py-1">
        <span className="text-ash-muted">意图:</span>
        <span className="px-2 py-0.5 bg-ash-primary/20 text-ash-primary rounded text-xs font-medium">
          {result.intent}
        </span>
      </div>
    );
  ...
  if (result.method) {
    items.push(
      <div key="method" className="flex items-center gap-2 py-1">
        <span className="text-ash-muted">分类方法:</span>
        <span className="text-ash-text">{result.method}</span>
      </div>
    );
  }
  ...
  if (result.confidence !== undefined) {
    items.push(
      <div key="confidence" className="flex items-center gap-2 py-1">
        <span className="text-ash-muted">置信度:</span>
        {formatConfidence(result.confidence)}
        <div className="flex-1 h-1.5 bg-ash-border rounded-full overflow-hidden ml-2">
          <div
    ...
    if (selected.length > 0 || scored.length > 0) {
      items.push(
        <div key="agent-selection" className="py-1">
          <div className="text-ash-muted mb-1">Agent 选择依据:</div>
          {selected.length > 0 && (
          ...
          {scored.length > 0 && (
            <div className="space-y-1">
              {scored.map((row, idx) => (
                <div key={`${row.agent || 'agent'}-${idx}`} className="text-[11px] text-ash-text/90">
                  <div className="font-medium">
                    {row.agent || `agent_${idx + 1}`}
                    {typeof row.score === 'number' ? ` (${row.score.toFixed(2)})` : ''}
                  </div>
                  {Array.isArray(row.reasons) && row.reasons.length > 0 && (
                    <div className="text-ash-muted pl-2">{'\u2022'} {row.reasons.join(' \u00b7 ')}</div>
                  )}
                </div>
              ))}
            </div>
  ...
  if (result.agent || result.agent_name) {
    items.push(
      <div key="agent" className="flex items-center gap-2 py-1">
        <span className="text-ash-muted">执行Agent:</span>
  ...
  if (result.tool || result.tools) {
    ...
    items.push(
      <div key="tools" className="flex items-center gap-2 py-1 flex-wrap">
        <span className="text-ash-muted">工具:</span>
  ...
  if (result.reasoning) {
    items.push(
      <div key="reasoning" className="py-1">
        <span className="text-ash-muted">推理:</span>
        <p className="text-ash-text/80 text-xs mt-1 pl-2 border-l-2 border-ash-primary/30">
          {result.reasoning}
        </p>
      </div>
    );
  ...
  if (result.data_sources || result.sources) {
    ...
    items.push(
      <div key="sources" className="flex items-center gap-2 py-1 flex-wrap">
        <span className="text-ash-muted">数据源:</span>
        {sources.map((src: string, i: number) => (
          <span key={i} className="px-2 py-0.5 bg-ash-panel text-ash-muted rounded text-xs">
            {src}
          </span>
        ))}
      </div>
  ...
  traceBlocks.forEach((block) => {
    ...
    items.push(
      <div key={`trace-${block.label}`} className="pt-1">
        <div className="text-2xs text-ash-muted uppercase mb-1">{block.label}</div>
  ...
  if (items.length === 0) {
    // 如果没有特定字段，显示原始 JSON
    return (
      <pre className="mt-1 text-xs bg-ash-bg p-2 rounded overflow-x-auto max-h-32">
...
export const ThinkingStepDetail: React.FC<ThinkingStepDetailProps> = ({
  ...
  return (
    <div className="px-3 pb-2 pt-1 border-t border-ash-border/30 bg-ash-bg/30">
      {traceViewMode === 'user' ? (
        <div className="space-y-2">
          {renderDecisionSummary(result) || (
            <div className="text-[11px] text-ash-muted">暂无更多可解释信息。</div>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          {(() => {
            ...
            if (Object.keys(expertSnapshot).length === 0) {
              return <div className="text-[11px] text-ash-muted">暂无专家层详情。</div>;
            }
            return (
              <pre className="text-2xs bg-ash-panel/60 p-2 rounded max-h-40 overflow-auto">
