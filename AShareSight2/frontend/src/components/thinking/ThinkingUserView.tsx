    ...
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle
          ...
          r={radius}
          stroke="currentColor"
          className="text-ash-primary transition-[stroke-dashoffset] duration-500 ease-out"
          strokeWidth={stroke}
          fill="none"
      ...
      <div className="absolute inset-0 flex items-center justify-center text-[10px] font-semibold text-ash-primary tabular-nums">
        {Math.round(percent)}
      </div>
    </div>
  );
      ...
      className={`
        ...
        ${isDone
          ...
          : isError
            ...
            : 'border-ash-primary/30 bg-ash-primary/5'}
      `}
    >
      <div className="flex items-center gap-1.5">
        ...
        {agent.status === 'running' && <Loader2 size={10} className="animate-spin text-ash-primary shrink-0" />}
        <span className={`flex-1 truncate font-medium ${isDone ? 'text-emerald-300' : isError ? 'text-red-300' : 'text-ash-primary'}`}>
          {agent.displayName}
        </span>
        {typeof conf === 'number' && (
          <span className="text-ash-muted tabular-nums text-[9px]">{conf}%</span>
        )}
      </div>
      {/* 进度条 */}
      <div className="h-[2px] rounded-full overflow-hidden bg-ash-border/60">
        <div
          className={`h-full rounded-full transition-[width] duration-500
            ${isDone ? 'bg-gradient-to-r from-ash-primary to-emerald-400' :
              isError ? 'bg-red-400' :
              'bg-gradient-to-r from-ash-primary via-cyan-400 to-ash-primary animate-pulse'}`}
          style={{ width: `${typeof conf === 'number' ? conf : isDone ? 100 : 60}%` }}
        />
      </div>
      {typeof agent.duration === 'number' && (
        <div className="flex items-center justify-between text-[9px] text-ash-muted tabular-nums">
 ...
 *
 * 设计基线：
 *   - 时间轴竖线：ash-primary → cyan-400 渐变
 *   - 阶段节点：done/active/pending 三态视觉
 *   - Agent 卡片：迷你进度条 + 置信度
 *   - 主题色严格使用 ash-primary（蓝色），辅以 cyan、emerald
 */
export const ThinkingUserView: React.FC<ThinkingUserViewProps> = ({ thinking }) => {
  ...
  return (
    <div className="mt-3 space-y-3 animate-fade-in">
      {/* === Header === */}
      <div className="flex items-center gap-3 px-3 py-2 rounded-xl border border-ash-border/60 bg-ash-panel/40 backdrop-blur-sm">
        <ProgressRing percent={progressPercent} />
        <div className="flex-1 min-w-0">
          <div className="text-xs font-semibold text-ash-text leading-tight flex items-center gap-1.5">
            <Zap size={11} className="text-ash-primary" />
            分析过程
          </div>
          <div className="text-[10px] text-ash-muted mt-0.5 tabular-nums">
            {doneCount}/{phases.length} 阶段 · {thinking.length} 个事件
            {!allDone && anyActive && <span className="ml-1 text-ash-primary/80">· 进行中</span>}
          </div>
        </div>
        <div className="flex items-center gap-1 text-[10px] shrink-0">
          {elapsed && (
            <span className="px-1.5 py-0.5 rounded-full border border-ash-primary/25 bg-ash-primary/10 text-ash-primary tabular-nums">
              ⏱ {elapsed}s
            </span>
          )}
        </div>
      ...
      <div className="relative pl-5 pr-1">
        {/* 主轴竖线 —— AShareSight 主题色 */}
        <div
          aria-hidden
          className="absolute left-[7px] top-2 bottom-2 w-[2px] rounded-full bg-gradient-to-b from-ash-primary via-cyan-400/70 to-ash-primary/20 opacity-60"
        />
            ...
            <div key={phase.id} className="relative pb-2 last:pb-0">
              ...
              <div
                className={`
                  ...
                  ${phase.status === 'done'
                    ...
                    : phase.status === 'active'
                      ? 'bg-ash-primary/15 border-2 border-ash-primary text-ash-primary animate-pulse'
                      : 'bg-ash-bg border-2 border-ash-border'}
                `}
                style={
                  phase.status === 'active'
                    ? { boxShadow: '0 0 0 3px rgb(var(--ash-primary) / 0.15)' }
                    : undefined
                }
              >
                {phase.status === 'done' && '✓'}
                {phase.status === 'active' && (
                  <span className="w-1.5 h-1.5 rounded-full bg-ash-primary" />
                )}
              </div>
              ...
              <button
                ...
                className={`
                  ...
                  ${isClickable ? 'cursor-pointer hover:bg-ash-hover/40' : 'cursor-default'}
                  ${phase.status === 'done'
                    ? 'text-ash-text'
                    : phase.status === 'active'
                      ? 'text-ash-primary'
              ...
              >
                ...
                {hasSteps && (
                  <span className="text-[9px] text-ash-muted/70 tabular-nums">
                ...
                {phase.duration !== null && (
                  <span
                    className={`text-[10px] tabular-nums shrink-0 ${
                      phase.duration > 3 ? 'text-amber-400' : 'text-ash-muted'
                    }`}
                  >
                ...
                {isClickable && (
                  isExpanded ? (
                    <ChevronDown size={10} className="text-ash-muted/60 shrink-0" />
                  ) : (
                    <ChevronRight size={10} className="text-ash-muted/60 shrink-0" />
                  )
                )}
              </button>
              ...
              {/* 展开 —— 子步骤 + agent 网格 */}
              {isExpanded && (
                <div className="ml-4 mt-1 pl-3 border-l border-dashed border-ash-border/50 space-y-0.5 animate-fade-in">
                  {phase.steps.map((step, idx) => (
                    ...
                    >
                      {step.isDone ? (
                        ...
                        <Clock size={9} className="text-ash-primary/70 shrink-0 mt-[3px]" />
                      )}
                      <span className={`flex-1 ${step.isDone ? 'text-ash-muted' : 'text-ash-text/85'}`}>
                        {step.message}
                        {step.agentName && (
                      ...
                      <span className="text-ash-muted/40 tabular-nums shrink-0 text-[9px] mt-[3px]">
      ...
      {agents.length > 0 && !expandedPhases.has('execute') && (
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5 text-[10px] text-ash-muted px-1">
            <Sparkles size={10} className="text-ash-primary/70" />
            <span>分析师团队</span>
      ...
      {statusText && anyActive && !cancelledStep && (
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11px] bg-ash-primary/8 text-ash-primary border border-ash-primary/20 animate-fade-in">
