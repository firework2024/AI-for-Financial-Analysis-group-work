  ...
  return (
    <div className="mt-2 space-y-2 max-h-96 overflow-y-auto">
      {thinking.map((step, index) => (
        <div
          key={index}
          className="text-xs bg-ash-panel/50 rounded-lg border border-ash-border/50 overflow-hidden"
        >
          {/* 步骤头部按钮 */}
          <button
            onClick={() => onToggleStep(index)}
            className="w-full p-2 flex items-start gap-2 hover:bg-ash-panel/80 transition-colors"
          >
            <span className="mt-0.5">{getStageIcon(step.stage)}</span>
            <div className="flex-1 min-w-0 text-left">
              <div className="font-medium text-ash-text flex items-center gap-2">
                {formatLangGraphStage(step.stage) || stageLabels[step.stage] || step.stage}
                {step.result?.confidence !== undefined && (
                  <span className="text-2xs px-1.5 py-0.5 bg-ash-bg rounded">
                    置信度: {formatConfidence(step.result.confidence)}
                  </span>
                )}
              </div>
              {step.message && (
                <div className="text-ash-muted mt-0.5 truncate">{step.message}</div>
              )}
            </div>
            <div className="flex items-center gap-2">
              <span className="text-ash-muted/50 text-2xs">
                {new Date(step.timestamp).toLocaleTimeString()}
              </span>
              {step.result && (
                <ChevronDown
                  size={12}
                  className={`text-ash-muted transition-transform ${expandedSteps.has(index) ? 'rotate-180' : ''}`}
                />
              )}
            </div>
          </button>
