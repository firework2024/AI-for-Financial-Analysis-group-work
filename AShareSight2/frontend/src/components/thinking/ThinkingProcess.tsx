  ...
  return (
    <div className="mt-3 border-t border-ash-border pt-3">
      ...
      <button
        ...
        className="flex items-center justify-between w-full text-xs text-ash-muted hover:text-ash-text transition-colors"
      >
        <div className="flex items-center gap-2">
          {isUserMode ? (
            <Sparkles size={14} className="text-emerald-400" />
          ) : (
            <Brain size={14} className="text-ash-primary" />
          )}
          ...
          {isUserMode ? (
            ...
            <span className="text-ash-muted">({thinking.length} 步骤)</span>
          )}
          {/* 可点击的视图模式切换 chip */}
          <span
            role="button"
            ...
            className="text-2xs px-1.5 py-0.5 rounded bg-ash-panel border border-ash-border/60 text-ash-muted hover:bg-ash-primary/10 hover:border-ash-primary/40 hover:text-ash-primary cursor-pointer transition-colors select-none"
          >
