...
export const getStageIcon = (stage: string) => {
  ...
  if (stage.includes('start') || stage.includes('processing') || stage.includes('collection')) return <Brain size={14} className="text-ash-primary animate-pulse" />;
  return <Brain size={14} className="text-ash-muted" />;
};
...
export const renderDecisionSummary = (result: any): React.ReactNode => {
  ...
  return (
    <div className="mb-2 text-[11px] text-ash-text bg-ash-panel/40 border border-ash-border/40 rounded px-2 py-1.5">
      {lines.map((line, idx) => (
...
export const renderTraceSteps = (steps: any[], depth = 0, showPayload = true): React.ReactNode => {
  ...
  return (
    <div className={`space-y-1 ${depth > 0 ? 'ml-3 border-l border-ash-border/40 pl-2' : ''}`}>
      {steps.map((step, index) => {
        ...
        return (
          <details key={`${depth}-${index}`} className="group rounded-md border border-ash-border/40 bg-ash-bg/40 px-2 py-1">
            <summary className="cursor-pointer list-none flex items-start gap-2 text-[11px] text-ash-text">
              <span className="mt-0.5">{getStageIcon(String(step?.stage || step?.state || step?.name || 'step'))}</span>
              <div className="flex-1 min-w-0">
                <div className="font-medium truncate">{label}</div>
                {summary && <div className="text-2xs text-ash-muted truncate">{summary}</div>}
              </div>
              {timestamp && <span className="text-2xs text-ash-muted">{timestamp}</span>}
            </summary>
