...
export const SubscribeModal: React.FC<SubscribeModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Dialog
      open={isOpen}
      onClose={onClose}
      panelClassName="bg-ash-panel border border-ash-border rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto shadow-2xl"
    >
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-ash-border bg-ash-panel p-4 rounded-t-2xl">
        <div className="flex items-center gap-2">
          <div className="rounded-lg bg-ash-primary/10 p-2">
            <Bell size={18} className="text-ash-primary" />
          </div>
          <div>
            <h2 className="text-lg font-semibold text-ash-text">订阅管理</h2>
            <p className="text-xs text-ash-muted">管理价格提醒与新闻提醒</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="space-y-5 p-6">
        <div className="grid gap-4 text-sm md:grid-cols-2">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-ash-text-secondary">股票代码（逗号或空格分隔）</label>
            <textarea
              rows={2}
              value={form.tickers}
              onChange={(event) => setField('tickers', event.target.value)}
              placeholder="AAPL, MSFT, TSLA"
              className="w-full resize-none rounded-lg border border-ash-border bg-ash-bg px-3 py-2.5 text-sm uppercase text-ash-text outline-none transition-colors focus:border-ash-primary focus:ring-1 focus:ring-ash-primary/30"
            />
          </div>

          <div className="space-y-2">
            <label className="text-xs font-medium text-ash-text-secondary">提醒模式</label>
            ...
            {form.alert_mode === 'price_target' && (
              <p className="text-2xs text-ash-muted">方向由后端根据当前价格自动推断（仅 above / below）</p>
            )}
          </div>

          <div className="space-y-2">
            <label className="text-xs font-medium text-ash-text-secondary">订阅类型</label>
            <div className="flex gap-4 pt-1">
              <label className="inline-flex cursor-pointer items-center gap-1.5 text-ash-text">
                <input
                  type="checkbox"
        ...
        <div className="border-t border-ash-border pt-5">
          <h4 className="mb-3 text-xs font-medium text-ash-muted">已订阅 ({subs.length})</h4>
          <div className="max-h-60 space-y-2 overflow-auto">
                  ...
                  className={[
                    ...
                    isDisabled
                      ...
                      : hasError
                        ...
                        : 'border-ash-border bg-ash-bg/60 hover:bg-ash-hover',
                  ].join(' ')}
                >
                  <div className="min-w-0 flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      <span className={isDisabled ? 'font-medium text-ash-muted line-through' : 'font-medium text-ash-text'}>
                        {sub.ticker}
                      </span>
                    ...
                    <div className="truncate text-ash-muted">{sub.email}</div>
                    <div className="text-ash-muted">
                  ...
                  <div className="ml-3 flex shrink-0 items-center gap-2">
                    <Button
                      ...
                      className={`p-1 ${isDisabled ? 'text-ash-muted hover:text-ash-success' : 'text-ash-success hover:text-ash-muted'}`}
            ...
            {subs.length === 0 && !subsLoading && <p className="py-4 text-center text-xs text-ash-muted">暂无订阅</p>}
          </div>
        </div>
      </div>
    </Dialog>
