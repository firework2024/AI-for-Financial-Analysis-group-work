 ...
 *
 * Persisted to localStorage (`asharesight-agent-preferences`) and synced to backend.
...
const STORAGE_KEY = 'asharesight-agent-preferences';
...
function depthButtonClass(depth: AgentDepth, isActive: boolean): string {
  if (!isActive) {
    return 'bg-ash-bg border border-ash-border text-ash-muted hover:text-ash-text';
  }
  if (depth === 'off') {
  ...
  return 'bg-ash-primary/10 text-ash-primary border border-ash-primary/30';
}
...
export const AgentControlPanel: React.FC = () => {
  ...
  return (
    <Card className="p-4 bg-ash-bg/40">
      <h3 className="text-sm font-medium text-ash-text mb-3">
        Agent 控制面板
      </h3>
      ...
      <div className="space-y-2 mb-4">
        {AGENT_NAMES.map(({ key, label }) => (
          <div key={key} className="flex items-center justify-between gap-3">
            <span className="text-xs text-ash-text min-w-20">{label}</span>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between text-xs mb-1">
          <span className="text-ash-muted">预算上限（轮次）</span>
          <span className="text-ash-text font-medium">{prefs.maxRounds}</span>
        </div>
        <input
          type="range"
          min={1}
          ...
          className="w-full accent-ash-primary h-1.5"
        />
        <div className="flex justify-between text-2xs text-ash-muted mt-0.5">
          <span>1</span>
          <span>10</span>
        </div>
      </div>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between text-xs mb-1">
          <span className="text-ash-muted">超时时间（秒）</span>
          <span className="text-ash-text font-medium">
        ...
        <input
          ...
          className="w-full rounded-md border border-ash-border bg-ash-bg px-2 py-1 text-xs text-ash-text outline-none focus:border-ash-primary"
      ...
      <p className="text-2xs text-ash-muted mt-3">
