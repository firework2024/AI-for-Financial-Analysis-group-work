  ...
  return (
    <section className="rounded-xl border border-ash-border bg-ash-card p-4">
      <div className="mb-3 flex flex-wrap items-end gap-3">
        ...
        <div>
          <label className="mb-1 block text-xs text-ash-muted">策略</label>
          <select
...
const MetricCard: React.FC<{ label: string; value: unknown }> = ({ label, value }) => (
  <div className="rounded-lg border border-ash-border bg-ash-bg p-3">
    <div className="text-xs text-ash-muted">{label}</div>
    <div className="text-base font-semibold text-ash-text">{value == null ? '-' : String(value)}</div>
  </div>
);
