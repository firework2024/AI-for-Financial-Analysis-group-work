  ...
  return (
    <section className="rounded-xl border border-ash-border bg-ash-card p-4">
      <div className="mb-3 flex flex-wrap items-end gap-3">
        <div>
          <label className="mb-1 block text-xs text-ash-muted">市场</label>
          <select
            className="rounded-lg border border-ash-border bg-ash-bg px-3 py-2 text-sm text-ash-text"
            value={market}
      ...
      <div className="max-h-72 overflow-auto rounded-lg border border-ash-border">
        <table className="w-full text-left text-sm">
          <thead className="sticky top-0 bg-ash-bg-secondary text-ash-text-secondary">
            <tr>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={String(item.symbol || item.name || Math.random())} className="border-t border-ash-border">
            ...
            {items.length === 0 && (
              <tr>
                <td className="px-3 py-6 text-center text-ash-muted" colSpan={4}>
                  暂无结果
                </td>
              </tr>
            )}
