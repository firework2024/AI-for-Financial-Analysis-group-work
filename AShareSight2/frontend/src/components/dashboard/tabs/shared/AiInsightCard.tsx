...
export function AiInsightCard({
  ...
  if (error && !insight) {
    return (
      <div className="bg-ash-card rounded-xl border border-ash-border p-4">
        <div className="flex items-center gap-2 text-ash-muted text-sm">
          <span>{TAB_ICONS[tab] ?? '🤖'}</span>
          <span>{TAB_LABELS[tab] ?? 'AI 分析'}</span>
        </div>
        <p className="text-ash-muted text-xs mt-2">
          AI 分析暂不可用
        </p>
      </div>
    );
  ...
  return (
    <div
      className={`bg-ash-card rounded-xl border p-4 transition-all duration-300 ${
        stale ? 'border-ash-border/50 opacity-90' : 'border-ash-border'
      }`}
    >
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-sm font-medium text-ash-text truncate">
              {label}
            </span>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {!insight.model_generated && (
              <span className="text-2xs text-ash-muted bg-ash-border/30 px-1.5 py-0.5 rounded">
                规则评分
              </span>
            )}
          </div>
        ...
        {onAskAbout && !diagnosticInsight && (
          <button
            ...
            className="p-1.5 rounded-lg text-ash-muted hover:bg-ash-primary/10 hover:text-ash-primary transition-all shrink-0"
            onClick={() => {
      ...
      {!compact && insight.summary && (
        <p className="text-xs text-ash-text/80 leading-relaxed mb-3 line-clamp-4">
          {insight.summary}
        </p>
      )}
      ...
      {!compact && tab === 'overview' && actionSuggestion?.action && (
        <div className="mb-3 rounded-lg border border-ash-border/60 bg-ash-border/20 p-2.5">
          <div className="text-xs font-semibold text-ash-text">
            操作建议：{actionSuggestion.action}
          </div>
          {actionSuggestion.rationale && (
            <div className="mt-1 text-2xs text-ash-muted leading-relaxed">
              {actionSuggestion.rationale}
            </div>
          )}
          <div className="mt-2 flex flex-wrap gap-1.5">
            {actionSuggestion.entryRange && (
              <span className="text-2xs text-ash-text bg-ash-card/70 border border-ash-border/60 rounded px-1.5 py-0.5">
      ...
      {insight.key_points.length > 0 && (
        <ul className="space-y-1 mb-2">
          {insight.key_points.map((point, i) => (
            <li
              key={i}
              className="flex items-start gap-1.5 text-xs text-ash-text/70"
            >
              <span className="text-ash-success mt-0.5 flex-shrink-0">•</span>
      ...
      {insight.risks.length > 0 && (
        <div className="mt-2 pt-2 border-t border-ash-border/50">
          {insight.risks.map((risk, i) => (
            <div
              key={i}
              className="flex items-start gap-1.5 text-xs text-ash-danger/80"
            >
      ...
      <div className="flex items-center justify-between mt-3 pt-2 border-t border-ash-border/30">
        <span className="text-2xs text-ash-muted">
          {insight.model_generated ? '基于 AI 分析' : '基于规则评分'}
        </span>
        {stale && (
          <span className="text-2xs text-ash-muted flex items-center gap-1">
