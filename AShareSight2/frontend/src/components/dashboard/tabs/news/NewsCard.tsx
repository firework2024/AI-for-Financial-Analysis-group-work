...
function getReliabilityTier(score: number): { label: string; className: string } {
  if (score >= 0.9) return { label: '权威', className: 'text-ash-success' };
  if (score >= 0.75) return { label: '可靠', className: 'text-ash-warning' };
  return { label: '', className: 'text-ash-muted' };
}
...
const DEFAULT_TAG_COLOR = 'bg-ash-border/30 text-ash-muted';
    ...
    <div
      ...
      className={`group p-3 rounded-lg border transition-all hover:shadow-sm ${
        isSelected
          ...
          : 'border-ash-border/50 hover:border-ash-border hover:bg-ash-hover/40'
    ...
    >
      ...
      <div className="flex items-start gap-2">
        {onToggleSelect && (
          <button
            ...
            className={`mt-0.5 h-4 w-4 rounded border flex items-center justify-center shrink-0 transition-colors ${
              isSelected
                ? 'bg-ash-primary border-ash-primary'
                : 'border-ash-border bg-transparent hover:border-ash-primary'
            }`}
          >
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-medium text-ash-text line-clamp-2 group-hover:text-ash-primary transition-colors">
            {news.title}
          </h4>

          {news.summary && (
            <p className="text-xs text-ash-muted mt-1 line-clamp-2">{news.summary}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-2">
        <div className="flex items-center gap-2 text-2xs text-ash-muted">
          <span>{formatNewsTime(news.ts)}</span>
          {typeof news.ranking_score === 'number' && (
            <>
              <span>·</span>
              <span className="text-ash-primary">评分 {news.ranking_score.toFixed(2)}</span>
            </>
          )}
          {typeof news.asset_relevance === 'number' && news.asset_relevance > 0 && (
            <>
              <span>·</span>
              <span className="text-ash-warning">相关 {(news.asset_relevance * 100).toFixed(0)}%</span>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-0.5">
          ...
          {onAnalyze && (
            <button
              ...
              className={`p-1.5 rounded-lg transition-all ${
                isAnalyzing
                  ? 'text-ash-muted opacity-50 cursor-not-allowed'
                  : 'text-ash-muted opacity-0 group-hover:opacity-100 hover:bg-amber-500/10 hover:text-amber-400'
              }`}
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-all ${
                isSelected
                  ...
                  : 'text-ash-muted opacity-0 group-hover:opacity-100 hover:bg-ash-primary/10 hover:text-ash-primary'
              }`}
            >
          ...
          {news.url && news.url !== '#' && (
            <ExternalLink
              size={13}
              className="text-ash-muted opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
            />
          )}
        </div>
      </div>
