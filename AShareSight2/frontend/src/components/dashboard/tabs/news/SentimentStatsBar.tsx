...
export function SentimentStatsBar({ news }: SentimentStatsBarProps) {
  ...
  const cards: { label: string; value: number; color: string; barColor: string }[] = [
    { label: '积极', value: stats.positive, color: 'text-ash-success', barColor: 'bg-ash-success' },
    { label: '中性', value: stats.neutral, color: 'text-ash-muted', barColor: 'bg-ash-muted' },
    { label: '消极', value: stats.negative, color: 'text-ash-danger', barColor: 'bg-ash-danger' },
  ];
      ...
      {cards.map((card) => (
        ...
        >
          <div className="flex items-baseline justify-between mb-2">
            <span className="text-xs text-ash-muted">{card.label}</span>
