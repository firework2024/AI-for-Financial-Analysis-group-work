...
function impactDot(level?: string) {
  if (level === 'high')   return 'bg-ash-danger';
  if (level === 'medium') return 'bg-ash-warning';
  return 'bg-ash-muted/50';
}
...
function NewsHighlightCard({ news }: { news: NewsItem[] }) {
  ...
  return (
    <div className="bg-ash-card rounded-xl border border-ash-border p-4">
      <div className="text-xs font-medium text-ash-muted mb-2.5">近期新闻</div>
          ...
          <li key={i} className="flex items-start gap-2">
            ...
            <div className="min-w-0">
              <a
                ...
                className="text-xs text-ash-text/85 hover:text-ash-primary line-clamp-2 leading-relaxed transition-colors"
              >
                {item.title}
              </a>
              <div className="mt-0.5 flex items-center gap-1.5 text-2xs text-ash-muted">
...
function PeerSnapshotCard({ peers, subjectSymbol }: { peers: PeerMetrics[]; subjectSymbol?: string }) {
  ...
  return (
    <div className="bg-ash-card rounded-xl border border-ash-border p-4">
      <div className="text-xs font-medium text-ash-muted mb-2.5">同行对比</div>
      <table className="w-full text-xs">
        <thead>
          <tr className="text-ash-muted border-b border-ash-border/50">
        ...
        <tbody className="divide-y divide-ash-border/20">
          {items.map((p) => (
            <tr key={p.symbol} className={p.symbol === subjectSymbol ? 'text-ash-primary' : ''}>
              <td className="py-1.5 font-medium truncate max-w-[70px]" title={p.name}>
                {p.symbol}
              </td>
              <td className="py-1.5 text-right text-ash-text/80">{fmtPE(p.trailing_pe ?? p.forward_pe)}</td>
              <td className="py-1.5 text-right text-ash-text/80">{fmtMktCap(p.market_cap)}</td>
            </tr>
          ))}
        </tbody>
      </table>
