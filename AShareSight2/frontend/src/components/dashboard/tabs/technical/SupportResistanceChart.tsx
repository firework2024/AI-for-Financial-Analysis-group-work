...
export function SupportResistanceChart({ technicals, marketChart }: SupportResistanceChartProps) {
  ...
  if (!option) {
    return (
      <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
        <div className="text-xs font-medium text-ash-muted mb-3">支撑/阻力位</div>
        <div className="text-sm text-ash-muted">暂无K线数据</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="text-xs font-medium text-ash-muted mb-2">K线图 · 支撑/阻力位</div>
      <ReactECharts
        option={option}
