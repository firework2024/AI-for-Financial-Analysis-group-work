...
export function TechnicalSubCharts({ indicatorSeries }: TechnicalSubChartsProps) {
  ...
  if (!rsiOption && !macdOption) {
    return (
      <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
        <div className="text-xs font-medium text-ash-muted mb-3">RSI / MACD</div>
        <div className="text-sm text-ash-muted">暂无技术指标时间序列数据</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {rsiOption && (
        <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
          <div className="text-xs font-medium text-ash-muted mb-2">
            RSI (14)
            <span className="ml-2 text-2xs text-ash-border">70 超买 / 30 超卖</span>
          </div>
          <ReactECharts
            option={rsiOption}
      ...
      {macdOption && (
        <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
          <div className="text-xs font-medium text-ash-muted mb-2">MACD (12, 26, 9)</div>
          <ReactECharts
            option={macdOption}
