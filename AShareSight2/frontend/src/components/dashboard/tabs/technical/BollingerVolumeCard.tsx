...
export function BollingerVolumeCard({ technicals }: BollingerVolumeCardProps) {
  ...
  return (
    <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="text-xs font-medium text-ash-muted mb-3">布林带 & 成交量</div>

      {/* Bollinger Bands */}
      <div className="mb-4">
        <div className="text-2xs font-medium text-ash-text-secondary mb-2">布林带 (20,2)</div>

        <div className="grid grid-cols-3 gap-3 mb-3">
          <div className="flex flex-col">
            <span className="text-2xs text-ash-muted">上轨</span>
            <span className="text-sm text-ash-danger tabular-nums">{fmtPrice(upper)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-2xs text-ash-muted">中轨</span>
            <span className="text-sm text-ash-text tabular-nums">{fmtPrice(middle)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-2xs text-ash-muted">下轨</span>
            <span className="text-sm text-ash-success tabular-nums">{fmtPrice(lower)}</span>
          </div>
        </div>

        {/* Position indicator */}
        <div className="mb-1.5">
          <div className="flex items-center justify-between text-2xs text-ash-muted mb-1">
            <span>下轨</span>
            <span>{position}</span>
            <span>上轨</span>
          </div>
          <div className="relative h-2 bg-ash-border rounded-full">
            <div
              className="absolute top-0 h-2 bg-gradient-to-r from-ash-success via-ash-warning to-ash-danger rounded-full opacity-30"
              style={{ width: '100%' }}
            />
            <div
              className="absolute -top-0.5 w-3 h-3 bg-ash-primary rounded-full border-2 border-ash-card"
      ...
      {/* Volume */}
      <div>
        <div className="text-2xs font-medium text-ash-text-secondary mb-2">成交量</div>
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col">
            <span className="text-2xs text-ash-muted">平均成交量</span>
            <span className="text-sm text-ash-text tabular-nums">{fmtVolume(avgVolume)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-2xs text-ash-muted">带宽</span>
            <span className="text-sm text-ash-text tabular-nums">
