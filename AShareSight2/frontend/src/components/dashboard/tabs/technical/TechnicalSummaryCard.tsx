...
const VERDICT_STYLES: Record<TechVerdict, { bg: string; text: string }> = {
  bullish: { bg: 'bg-ash-success/10', text: 'text-ash-success' },
  bearish: { bg: 'bg-ash-danger/10', text: 'text-ash-danger' },
  neutral: { bg: 'bg-ash-warning/10', text: 'text-ash-warning' },
};
...
export function TechnicalSummaryCard({ technicals }: TechnicalSummaryCardProps) {
  ...
  return (
    <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="text-xs font-medium text-ash-muted mb-3">技术面综合评估</div>
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3 text-2xs">
            <span className="text-ash-success">买入: {summary.counts.buy}</span>
            <span className="text-ash-danger">卖出: {summary.counts.sell}</span>
            <span className="text-ash-warning">中性: {summary.counts.neutral}</span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col">
          <span className="text-2xs text-ash-muted">趋势</span>
          <span className="text-sm text-ash-text font-medium mt-0.5">{summary.trend}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-2xs text-ash-muted">动量</span>
          <span className="text-sm text-ash-text font-medium mt-0.5">{summary.momentum}</span>
        </div>
      </div>
    </div>
  );
