...
const SIGNAL_DISPLAY: Record<MASignal, { label: string; className: string }> = {
  ...
  neutral: { label: '--', className: 'text-ash-muted' },
};
  ...
  if (rows.length === 0) {
    return (
      <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
        <div className="text-xs font-medium text-ash-muted mb-3">均线指标</div>
  ...
  return (
    <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-medium text-ash-muted">均线指标</span>
        {close != null && (
          <span className="text-2xs text-ash-text-secondary">
            当前价: <span className="text-ash-text font-medium tabular-nums">{fmtPrice(close)}</span>
          </span>
        )}
      </div>
          ...
          {rows.map((row) => {
            ...
            return (
              <tr key={row.label} className="border-b border-ash-border/50 last:border-b-0">
                ...
                <td className="text-right py-2 tabular-nums text-ash-text">{fmtPrice(row.value)}</td>
