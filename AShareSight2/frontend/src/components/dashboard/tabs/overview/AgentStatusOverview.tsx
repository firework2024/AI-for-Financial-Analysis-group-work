...
const STATUS_STYLE: Record<AgentStatusKind, { dot: string; text: string; label: string }> = {
  success: { dot: 'bg-ash-success', text: 'text-ash-success', label: '完成' },
  fallback: { dot: 'bg-ash-warning', text: 'text-ash-warning', label: '降级' },
  error: { dot: 'bg-ash-danger', text: 'text-ash-danger', label: '失败' },
  not_run: { dot: 'bg-ash-muted', text: 'text-ash-muted', label: '未运行' },
  unknown: { dot: 'bg-ash-muted', text: 'text-ash-muted', label: '未知' },
};
...
const DIAGNOSTIC_STYLE: Record<DiagnosticSeverity, { dot: string; text: string }> = {
  info: { dot: 'bg-ash-muted', text: 'text-ash-muted' },
  warn: { dot: 'bg-ash-warning', text: 'text-ash-warning' },
  error: { dot: 'bg-ash-danger', text: 'text-ash-danger' },
};
...
export function AgentStatusOverview({ reportData }: AgentStatusOverviewProps) {
  ...
  return (
    <div className="flex flex-col p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-medium text-ash-muted flex items-center gap-1">
          Agent 执行总览
          <CardInfoTip
        ...
        <span className="text-2xs text-ash-muted">{rows.length} agents</span>
      </div>
      ...
      <div className="space-y-2">
        {rows.map((row) => {
          ...
          return (
            <div key={row.key} className="rounded-lg border border-ash-border/70 p-2">
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full shrink-0 ${style.dot}`} />
                <span className="text-sm text-ash-text flex-1">{row.label}</span>
                {row.confidence != null && (
                  <span className="text-2xs text-ash-muted tabular-nums">
              ...
              {row.reasonKind && (
                <div className="mt-1 text-2xs text-ash-muted">
      ...
      {diagnostics.length > 0 && (
        <div className="mt-3 pt-3 border-t border-ash-border/70 space-y-2">
          <div className="text-2xs font-medium text-ash-muted">执行诊断</div>
            ...
            return (
              <div key={item.id} className="group/diag flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-2xs text-ash-muted leading-relaxed line-clamp-2">{item.detail}</div>
                </div>
                <CardInfoTip
                  icon="alert"
                  size={13}
