...
export function ScoreRing({ valuation, technicals, reportData, insightScore }: ScoreRingProps) {
  ...
  const ringColor =
    score >= 7 ? 'text-ash-success' : score >= 4 ? 'text-ash-warning' : 'text-ash-danger';

  return (
    <div className="flex flex-col items-center justify-center p-4 bg-ash-card rounded-xl border border-ash-border">
      ...
      <div className="relative w-32 h-32">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-2xs text-ash-muted">/10</span>
        </div>
      </div>
      <div className="text-2xs text-ash-muted mt-2" title={source === 'baseline' ? '基于实时市场数据自动计算' : ''}>
