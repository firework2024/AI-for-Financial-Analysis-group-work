  ...
  return (
    <div className="flex flex-col p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-semibold text-ash-text">{fearGreed.label}</span>
      </div>

      <div className="mt-2 flex items-end gap-2">
        <span className="text-3xl font-semibold text-ash-text tabular-nums">
          {fearGreed.value == null ? '--' : Math.round(fearGreed.value)}
        </span>
        <span className="text-xs text-ash-muted mb-1">/ 100</span>
      </div>

      <div className="mt-3">
        <div className="h-2 rounded-full bg-gradient-to-r from-ash-danger via-ash-warning to-ash-success relative">
          <span
            className="absolute top-1/2 h-3 w-3 -translate-y-1/2 rounded-full border border-ash-card bg-ash-text shadow"
            style={{ left: `calc(${pointerLeft}% - 6px)` }}
          />
        </div>
        <div className="mt-1 flex justify-between text-2xs text-ash-muted">
          <span>恐惧</span>
          <span>中性</span>
          <span>贪婪</span>
        </div>
      ...
      <div className="mt-3 rounded-lg border border-ash-border/60 bg-ash-border/20 p-2">
        <div className="text-2xs text-ash-muted">{fearGreed.source}</div>
        <div className="mt-1 text-xs text-ash-text/80 leading-relaxed line-clamp-3">{summaryText}</div>
      </div>

      {macroMetrics.length > 0 && (
        <div className="mt-3 grid grid-cols-2 gap-2">
          {macroMetrics.map((metric) => (
            <div key={metric.label} className="rounded-md border border-ash-border/50 px-2 py-1">
              <div className="text-2xs text-ash-muted">{metric.label}</div>
              <div className="text-xs text-ash-text tabular-nums">{metric.value}</div>
            </div>
          ))}
        </div>
      )}
