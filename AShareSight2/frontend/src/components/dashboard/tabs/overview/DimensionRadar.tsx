...
const statusTextClass = (item: Dimension): string => {
  if (item.status === 'not_run') return 'text-ash-muted';
  if (item.status === 'error') return 'text-ash-danger';
  return 'text-ash-text-secondary';
};

const progressClass = (item: Dimension): string => {
  if (item.status === 'not_run') return 'bg-ash-border';
  if (item.status === 'error') return 'bg-ash-danger';
  if (item.value >= 70) return 'bg-ash-success';
  if (item.value >= 40) return 'bg-ash-warning';
  if (item.value > 0) return 'bg-ash-danger';
  return 'bg-ash-border';
};
...
export function DimensionRadar({
  ...
  return (
    <div className="flex flex-col p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="flex items-center gap-1 text-xs font-medium text-ash-muted mb-3">
        分析维度覆盖
      ...
      <div className="space-y-2.5">
        {dimensions.map((item) => (
          <div key={item.name} className="flex items-center gap-2">
            <span className="text-2xs text-ash-muted w-16 shrink-0 text-right">{item.name}</span>
