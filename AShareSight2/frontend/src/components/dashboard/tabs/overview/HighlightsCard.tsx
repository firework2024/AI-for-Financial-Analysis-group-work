  ...
  return (
    <div className="flex flex-col p-4 bg-ash-card rounded-xl border border-ash-border">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <div className="text-2xs font-semibold text-ash-success mb-2">看多因素</div>
          {highlights.bullish.length === 0 ? (
            <div className="text-2xs text-ash-muted">--</div>
          ) : (
            <ul className="space-y-1.5">
              {highlights.bullish.map((item, idx) => (
                <li key={idx} className="flex items-start gap-1.5 text-2xs text-ash-text">
                  <span className="text-ash-success mt-0.5 shrink-0">+</span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
        ...
        <div>
          <div className="text-2xs font-semibold text-ash-danger mb-2">看空因素</div>
          {highlights.bearish.length === 0 ? (
            ...
            <ul className="space-y-1.5">
              {highlights.bearish.map((item, idx) => (
                <li key={idx} className="flex items-start gap-1.5 text-2xs text-ash-text">
                  <span className="text-ash-danger mt-0.5 shrink-0">-</span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
