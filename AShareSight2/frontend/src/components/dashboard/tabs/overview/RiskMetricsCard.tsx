...
const RISK_STYLES: Record<RiskLevel, { dot: string; text: string; badge: string; label: string }> = {
  low: {
    dot: 'bg-ash-success',
    text: 'text-ash-success',
    badge: 'bg-ash-success/10',
    label: '低',
  },
  medium: {
    dot: 'bg-ash-warning',
    text: 'text-ash-warning',
    badge: 'bg-ash-warning/10',
    label: '中',
  },
  high: {
    dot: 'bg-ash-danger',
    text: 'text-ash-danger',
    badge: 'bg-ash-danger/10',
    label: '高',
  },
};
...
export function RiskMetricsCard({ valuation, reportData }: RiskMetricsCardProps) {
  ...
  return (
    <div className="flex flex-col p-4 bg-ash-card rounded-xl border border-ash-border">
      ...
      {diagnosticCount > 0 && (
        <div className="text-2xs text-ash-muted mb-3 flex items-center gap-1">
          <span>
            检测到 {diagnosticCount} 条执行诊断信息，已迁移至 <span className="text-ash-text">Agent 执行总览</span>。
          </span>
          <CardInfoTip
            icon="alert"
            size={13}
        ...
        {financialRisks.map((risk, idx) => {
          ...
          return (
            <div key={`${risk.label}-${idx}`} className="flex items-center gap-2">
              ...
              <span className="text-sm text-ash-text flex-1 truncate">{risk.label}</span>
