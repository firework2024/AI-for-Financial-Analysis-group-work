...
const SIGNAL_STYLES: Record<Signal, { bg: string; text: string; label: string }> = {
  bullish: { bg: 'bg-ash-success/10', text: 'text-ash-success', label: '看多' },
  bearish: { bg: 'bg-ash-danger/10', text: 'text-ash-danger', label: '看空' },
  neutral: { bg: 'bg-ash-warning/10', text: 'text-ash-warning', label: '中性' },
};
...
export function AnalystRatingCard({ technicals, reportData }: AnalystRatingCardProps) {
  ...
  return (
    <div className="flex flex-col p-4 bg-ash-card rounded-xl border border-ash-border">
      ...
      <div className="text-2xs text-ash-text-secondary mt-2">
        {rating.description}
      </div>
          ...
          {rating.signals.map((s) => {
            ...
            return (
              <div key={s.name} className="flex items-center justify-between text-2xs">
                <span className="text-ash-muted">{s.name}</span>
