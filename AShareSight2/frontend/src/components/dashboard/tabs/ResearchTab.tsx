...
function renderFreshnessBadge(badge: FreshnessBadge) {
  const className = badge.tone === 'warning'
    ...
    : 'border-ash-border/80 bg-ash-card text-ash-text';
...
export function ResearchTab() {
  ...
  if (!reportData) {
    ...
    return (
      <div className="space-y-4">
        ...
        <div
          className="rounded-xl border border-ash-border bg-ash-card px-4 py-4"
          data-testid="research-empty-state"
          data-state={runningState ? 'running' : blockedState ? 'blocked' : 'empty'}
        >
          <div className="text-sm font-medium text-ash-text">
            {runningState
              ? '深度分析执行中'
              : blockedState
                ? '报告未发布（质量门禁拦截）'
          ...
          <div className="mt-1 text-xs text-ash-muted">
            {runningState
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-lg border border-ash-primary/40 bg-ash-primary/10 text-ash-primary hover:bg-ash-primary/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {runningState ? '执行中…' : '生成深度报告'}
            </button>
            <label className="flex items-center gap-1.5 px-2 py-1 text-2xs text-ash-muted border border-ash-border rounded-lg">
              <input
                type="checkbox"
          ...
          {error && (
            <div className="mt-2 text-xs text-ash-danger">
              生成失败：{error}
            </div>
          )}
        </div>
      ...
      <div className="space-y-4">
        ...
        >
          ...
          <button
            ...
            className="mt-3 px-3 py-1.5 rounded-lg border border-ash-danger/40 bg-ash-danger/15 text-ash-danger hover:bg-ash-danger/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            重新生成当前标的报告
          </button>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-ash-card rounded-xl border border-ash-border overflow-hidden">
        <button
          type="button"
          onClick={() => setReportExpanded((prev) => !prev)}
          className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-ash-text hover:bg-ash-border/10 transition-colors"
        >
          ...
          <span
            className={`text-ash-muted transition-transform duration-200 ${
        ...
        {reportExpanded && (
          <div className="border-t border-ash-border px-4 py-4 space-y-4">
