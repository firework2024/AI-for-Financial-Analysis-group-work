...
function contributionColor(value: number): string {
  if (value > 0) return 'bg-ash-success/80';
  if (value < 0) return 'bg-ash-danger/80';
  return 'bg-ash-muted/60';
}
...
export function ScoreExplainDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40" role="dialog" aria-modal="true">
      <div className="w-full max-w-md h-full bg-ash-card border-l border-ash-border shadow-2xl overflow-y-auto">
        <div className="sticky top-0 z-10 px-4 py-3 border-b border-ash-border bg-ash-card/95 backdrop-blur">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-semibold text-ash-text">{title} 评分构成</h3>
              <p className="text-2xs text-ash-muted mt-0.5">
        ...
        <div className="p-4 space-y-3">
          {breakdown.length === 0 ? (
            ...
            breakdown.map((item) => (
              <div key={item.factor_key} className="border border-ash-border rounded-lg p-3">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-xs font-medium text-ash-text">{item.label}</span>
                  <span className="text-2xs text-ash-muted">
                    权重 {(item.weight * 100).toFixed(0)}% · 贡献 {item.contribution.toFixed(2)}
                  </span>
                </div>
                <div className="h-1.5 rounded-full bg-ash-bg-secondary overflow-hidden">
                  <div
                    className={`h-full ${contributionColor(item.contribution)}`}
                    style={{ width: `${normalizeWidth(item, maxAbsContribution)}%` }}
                  />
                </div>
                <div className="mt-2 text-2xs text-ash-muted">
                  数值 {item.value.toFixed(2)}
                </div>
                {item.rationale && (
                  <div className="mt-1 text-2xs text-ash-text/80 leading-relaxed">
                    {item.rationale}
                  </div>
                )}
              </div>
