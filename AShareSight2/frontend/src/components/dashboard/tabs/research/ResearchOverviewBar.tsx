...
export function ResearchOverviewBar({
  ...
  if (loading && !overview) {
    return (
      <div className="bg-ash-card rounded-xl border border-ash-border p-5 animate-pulse">
        <div className="flex items-center gap-5">
          <div className="w-16 h-16 rounded-full bg-ash-border" />
          <div className="flex-1 space-y-2">
            <div className="h-5 w-48 bg-ash-border rounded" />
  ...
  return (
    <div className="bg-ash-card rounded-xl border border-ash-border p-5">
      <div className="flex items-center gap-5">
        {/* 大号评分环 */}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <h3 className="text-base font-semibold text-ash-text">
              综合评估
            </h3>
            ...
            <button
              type="button"
              className="ml-auto text-2xs text-ash-primary hover:text-ash-primary/80 transition-colors"
          ...
          {/* 摘要 */}
          <p className="text-xs text-ash-text/80 leading-relaxed line-clamp-3">
            {overview.summary}
          </p>
          ...
          {overview.key_metrics && overview.key_metrics.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-2">
              {overview.key_metrics.map((m, i) => (
                <div
                  key={i}
                  className="flex items-center gap-1 text-2xs text-ash-muted"
                >
                  <span className="text-ash-text/60">{m.label}:</span>
                  <span className="font-medium text-ash-text">{m.value}</span>
                </div>
              ))}
            </div>
          )}
