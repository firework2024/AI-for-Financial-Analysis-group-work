    ...
    <div className="space-y-2" id="research-reference-list">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-ash-text">
          参考来源
          <span className="ml-2 text-xs text-ash-muted font-normal">({citations.length})</span>
        </h4>
        <button
          type="button"
      ...
      <div className="bg-ash-card border border-ash-border rounded-lg divide-y divide-ash-border">
            ...
            <div className="flex-1 min-w-0">
              {ref.url ? (
                <a
                  ...
                  className="text-sm text-ash-primary hover:underline truncate block"
                >
                  {ref.source}
                </a>
              ) : (
                <span className="text-sm text-ash-text truncate block">{ref.source}</span>
              )}
            </div>
            <span className="ml-3 shrink-0 text-xs text-ash-muted">
              {ref.count} 次引用
            </span>
          </div>
        ))}
      ...
      {showSnippets && (
        <div
          ref={snippetRef}
          className="rounded-lg border border-ash-border bg-ash-card/70"
          data-testid="research-reference-snippets"
        >
          <div className="flex items-center justify-between px-3 py-2 border-b border-ash-border/60">
            <span className="text-xs font-medium text-ash-text">证据片段</span>
