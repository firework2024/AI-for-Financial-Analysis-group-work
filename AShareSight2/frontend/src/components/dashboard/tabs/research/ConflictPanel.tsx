...
export function ConflictPanel({ report }: ConflictPanelProps) {
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold text-ash-text">
        观点冲突
        {hasAnyConflict && (
          <span className="ml-2 text-xs text-ash-warning font-normal">
            (已检测)
          </span>
        )}
      </h4>
      ...
      {legacyConflicts.map((conflict, idx) => (
        <div
          key={`conflict-${idx}`}
          className="bg-ash-card border border-ash-border rounded-lg p-4"
        >
          {conflict.topic ? (
            <div className="text-sm font-medium text-ash-text mb-2">{conflict.topic}</div>
          ) : null}
          {conflict.description ? (
            <p className="text-xs text-ash-muted mb-3">{conflict.description}</p>
          ) : null}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded-lg bg-ash-success/5 border border-ash-success/20 p-3">
              <div className="text-xs font-semibold text-ash-success mb-1">看多观点</div>
              <p className="text-xs text-ash-text-secondary leading-relaxed">{conflict.bullish ?? '--'}</p>
            </div>
            <div className="rounded-lg bg-ash-danger/5 border border-ash-danger/20 p-3">
              <div className="text-xs font-semibold text-ash-danger mb-1">看空观点</div>
              <p className="text-xs text-ash-text-secondary leading-relaxed">{conflict.bearish ?? '--'}</p>
            </div>
          </div>
        </div>
      ))}
      ...
      {!hasMatrix && disclosure && (
        <div className="rounded-lg border border-ash-warning/30 bg-ash-warning/5 p-3">
          <div className="text-xs font-semibold text-ash-warning mb-1">冲突披露</div>
          <pre className="text-2xs text-ash-text/80 whitespace-pre-wrap leading-relaxed">
            {disclosure}
          </pre>
        </div>
      )}
