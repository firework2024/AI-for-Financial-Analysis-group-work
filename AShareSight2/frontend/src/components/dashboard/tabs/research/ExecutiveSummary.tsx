...
export function ExecutiveSummary({ report }: ExecutiveSummaryProps) {
  ...
  return (
    <div className="bg-ash-card border border-ash-border rounded-lg p-4">
      <h4 className="text-sm font-semibold text-ash-text mb-2">摘要总结</h4>
      <div className="text-sm text-ash-text-secondary leading-relaxed prose prose-sm prose-slate dark:prose-invert max-w-none">
      ...
      {needsTruncation ? (
        <button
          ...
          className="mt-2 text-xs text-ash-primary hover:underline"
        >
          {expanded ? '收起' : '展开全文'}
        </button>
      ) : null}
