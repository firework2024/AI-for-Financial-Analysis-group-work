...
export function ResearchMetadata({ reportData, loading }: ResearchMetadataProps) {
  if (loading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="bg-ash-card border border-ash-border rounded-lg p-3 animate-pulse">
            <div className="h-3 bg-ash-border rounded w-16 mb-2" />
            <div className="h-5 bg-ash-border rounded w-12" />
          </div>
        ))}
      </div>
    );
  ...
  if (!reportData) {
    return (
      <div className="bg-ash-card border border-ash-border rounded-lg p-4 text-center">
        <p className="text-sm text-ash-muted">尚未生成研究报告</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
      {metrics.map((metric) => (
        <div
          key={metric.label}
          className="bg-ash-card border border-ash-border rounded-lg p-3"
        >
          <div className="text-xs text-ash-muted mb-1">{metric.label}</div>
          <div className={`text-lg font-semibold ${metric.tone === 'warning' ? 'text-ash-warning' : 'text-ash-text'}`}>
            {metric.value}
          </div>
          {metric.subLabel ? (
            <div className="text-2xs text-ash-muted mt-0.5">{metric.subLabel}</div>
          ) : null}
        </div>
      ))}
    </div>
