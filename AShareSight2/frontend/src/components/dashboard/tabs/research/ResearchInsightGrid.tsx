...
function MetricTag({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between py-1 border-b border-ash-border/20 last:border-b-0">
      <span className="text-2xs text-ash-muted truncate">{label}</span>
      <span className="text-2xs font-semibold text-ash-text ml-2 whitespace-nowrap">
        {value}
      </span>
    </div>
  );
...
function ResearchCard({
  ...
  return (
    <div
      className={`bg-ash-card rounded-xl border p-4 flex flex-col transition-all duration-300 hover:border-ash-primary/30 ${
    ...
    >
      ...
      {/* 摘要 */}
      {insight.summary && (
        <p className="text-xs text-ash-text/80 leading-relaxed line-clamp-3 mb-2">
          {insight.summary}
        </p>
      )}
      ...
      {insight.key_points.length > 0 && (
        <ul className="space-y-0.5 mb-2">
          {insight.key_points.slice(0, 3).map((point, i) => (
            <li
              key={i}
              className="flex items-start gap-1.5 text-2xs text-ash-text/70"
            >
      ...
      {insight.risks.length > 0 && (
        <div className="pt-2 border-t border-ash-border/40 mt-2">
          {insight.risks.slice(0, 2).map((risk, i) => (
            <div
              key={i}
              className="flex items-start gap-1.5 text-2xs text-ash-danger/80"
            >
      ...
      {/* 来源标签 */}
      <div className="flex items-center justify-between mt-2 pt-1.5 border-t border-ash-border/20">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="text-2xs text-ash-primary hover:text-ash-primary/80 transition-colors"
            onClick={() => onOpenScoreExplain?.(insight, config.label)}
          >
            查看构成
          </button>
          {stale && (
            <span className="text-2xs text-ash-muted flex items-center gap-0.5">
...
export function ResearchInsightGrid({
  ...
  if (error && !insights) {
    return (
      <div className="bg-ash-card rounded-xl border border-ash-border p-6 text-center">
        <p className="text-sm text-ash-muted">AI 洞察暂不可用</p>
        <p className="text-xs text-ash-muted/60 mt-1">{error}</p>
      </div>
    );
  }
