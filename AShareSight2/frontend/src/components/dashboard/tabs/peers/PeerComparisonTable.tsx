        ...
        <thead>
          <tr className="border-b border-ash-border">
            {COLUMNS.map((col) => (
              <th
                key={col.key}
                className={`px-3 py-2 text-xs font-medium text-ash-muted whitespace-nowrap ${
        ...
        <tbody>
          {peers.map((peer) => {
            ...
            return (
              <tr
                key={peer.symbol}
                className={`border-b border-ash-border/50 transition-colors ${
                  isCurrent
                    ? 'bg-ash-primary/5'
                    : 'hover:bg-ash-hover/40'
                }`}
              >
                {COLUMNS.map((col) => (
                  <td
                    ...
                    className={`px-3 py-2 whitespace-nowrap ${
                      ...
                      col.key === 'symbol' && isCurrent
                        ? 'text-ash-primary font-semibold'
                        : 'text-ash-text'
                    }`}
                  >
