...
export function AiPeerSummary({ reportData, loading }: AiPeerSummaryProps) {
  if (loading) {
    return (
      <div className="bg-ash-card border border-ash-border rounded-lg p-4 animate-pulse">
        <div className="h-4 bg-ash-border rounded w-32 mb-3" />
        <div className="space-y-2">
          <div className="h-3 bg-ash-border rounded w-full" />
          <div className="h-3 bg-ash-border rounded w-3/4" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-ash-card border border-ash-border rounded-lg p-4">
      <h4 className="text-sm font-semibold text-ash-text mb-2">AI 同行分析</h4>
      {summary ? (
        <p className="text-sm text-ash-text-secondary leading-relaxed whitespace-pre-wrap">
          {summary}
        </p>
      ) : (
