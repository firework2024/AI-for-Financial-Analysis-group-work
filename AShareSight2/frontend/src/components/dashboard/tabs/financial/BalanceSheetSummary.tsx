...
export function BalanceSheetSummary({ financials }: BalanceSheetSummaryProps) {
  ...
  return (
    <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="text-xs font-medium text-ash-muted mb-3">资产负债概要</div>

      <div className="grid grid-cols-2 gap-4">
        {items.map((item) => (
          <div key={item.label} className="flex flex-col">
            <span className="text-2xs text-ash-muted">{item.label}</span>
            <span className="text-sm font-semibold text-ash-text tabular-nums mt-0.5">
              {item.value}
            </span>
            {item.subtext && (
              <span className="text-2xs text-ash-muted mt-0.5">{item.subtext}</span>
            )}
          </div>
        ))}
      </div>
