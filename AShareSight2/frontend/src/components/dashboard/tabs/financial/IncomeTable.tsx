...
function yoyClass(values: (number | null)[], index: number): string {
  ...
  if (change > 0.05) return 'text-ash-success';
  if (change < -0.05) return 'text-ash-danger';
  return '';
}
...
export function IncomeTable({ financials }: IncomeTableProps) {
  ...
  if (!financials || periods.length === 0) {
    return (
      <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
        <div className="text-xs font-medium text-ash-muted mb-3">利润表</div>
  ...
  return (
    <div className="p-4 bg-ash-card rounded-xl border border-ash-border overflow-x-auto">
      <div className="text-xs font-medium text-ash-muted mb-3">利润表</div>

      <table className="w-full text-2xs">
        <thead>
          <tr className="border-b border-ash-border">
            <th className="text-left py-2 pr-4 text-ash-muted font-medium whitespace-nowrap sticky left-0 bg-ash-card">
              指标
            </th>
            {periods.map((p) => (
              <th key={p} className="text-right py-2 px-2 text-ash-muted font-medium whitespace-nowrap">
                {p}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {ROWS.map((row) => {
            ...
            return (
              <tr key={row.key} className="border-b border-ash-border/50 last:border-b-0">
                <td className="py-2 pr-4 text-ash-text font-medium whitespace-nowrap sticky left-0 bg-ash-card">
                  {row.label}
                </td>
