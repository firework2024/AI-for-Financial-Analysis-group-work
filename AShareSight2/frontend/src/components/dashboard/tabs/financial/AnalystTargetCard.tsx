...
export function AnalystTargetCard({ targets, recommendations, currentPrice }: AnalystTargetCardProps) {
  ...
  if (!option && !recBar) {
    return (
      <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
        ...
        <div className="text-sm text-ash-muted">暂无分析师数据</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 bg-ash-card rounded-xl border border-ash-border">
      <div className="flex items-center gap-1 text-xs font-medium text-ash-muted mb-2">
        分析师目标价
        <CardInfoTip content="来源：yfinance 分析师目标价预测 + 评级分布" />
      </div>
      ...
      {recBar && (
        <div className="mt-3">
          <div className="text-2xs text-ash-muted mb-1.5">评级分布 ({recBar.total} 位分析师)</div>
          ...
          <div className="flex justify-between mt-1 text-2xs text-ash-muted">
            <span>买入</span>
            <span>卖出</span>
          </div>
        </div>
