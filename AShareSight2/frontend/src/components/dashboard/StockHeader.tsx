...
export function StockHeader({
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-5 py-3 bg-ash-card border-b border-ash-border shrink-0 max-lg:px-3 max-lg:flex-wrap max-lg:gap-2">
      {/* Left: Symbol info + Price */}
      <div className="flex items-center gap-4 min-w-0 max-lg:gap-2">
        {/* Symbol + Name */}
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-lg font-bold text-ash-text truncate">{displayName || ticker}</span>
          <span className="text-2xs text-ash-muted bg-ash-bg-secondary px-2 py-0.5 rounded shrink-0 uppercase">
            {assetType}
          </span>
        </div>
        ...
        {loading ? (
          <div className="h-6 w-24 bg-ash-border rounded animate-pulse" />
        ) : (
          <div className="flex items-center gap-3 shrink-0">
            {closePrice !== null && (
              <span className="text-lg font-semibold text-ash-text tabular-nums">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          ...
          className={`p-2 rounded-lg border transition-colors ${
            isInWatchlist
              ? 'border-ash-warning/50 bg-ash-warning/10 text-ash-warning'
              : 'border-ash-border bg-ash-bg text-ash-muted hover:text-ash-warning hover:border-ash-warning/50'
          }`}
        ...
        <button
          ...
          disabled={isRunning}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-ash-border text-ash-muted hover:text-ash-primary hover:border-ash-primary/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isRunning && runId ? (
        ...
        <button
          ...
          disabled={isRunning}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-ash-primary/40 bg-ash-primary/10 text-ash-primary hover:bg-ash-primary/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isRunning && runId ? (
            <Loader2 size={12} className="animate-spin" />
          ) : (
