...
function staleClass(level: SourceRow['staleLevel']): string {
  if (level === 'critical') return 'bg-ash-danger/10 text-ash-danger border-ash-danger/40';
  if (level === 'warn') return 'bg-ash-warning/10 text-ash-warning border-ash-warning/40';
  if (level === 'ok') return 'bg-ash-success/10 text-ash-success border-ash-success/40';
  return 'bg-ash-hover text-ash-muted border-ash-border';
}
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-ash-border bg-ash-bg hover:bg-ash-hover transition-colors text-ash-text-secondary text-xs"
        title="查看数据来源与降级信息"
      >
        <Info size={14} />
        <span>数据来源</span>
        {(summary.critical > 0 || summary.fallback > 0) && (
          <span className="inline-flex items-center gap-1 rounded-full bg-ash-danger/10 text-ash-danger px-1.5 py-0.5 text-[10px]">
      ...
      <Dialog
        ...
        panelClassName="w-full max-w-4xl max-h-[82vh] overflow-hidden bg-ash-card border border-ash-border rounded-xl shadow-2xl"
      >
        <div className="p-4 border-b border-ash-border flex items-center justify-between">
          <div>
            <h3 id="dashboard-source-title" className="text-base font-semibold text-ash-text">
              数据来源与可追踪信息
            </h3>
          ...
          <button
            ...
            className="p-1.5 rounded-md border border-ash-border text-ash-muted hover:text-ash-text hover:bg-ash-hover"
            aria-label="关闭数据来源弹窗"
          >
            <XCircle size={16} />
          </button>
        ...
        <div className="p-4 overflow-y-auto max-h-[70vh] space-y-3">
          {rows.map((row) => (
            <section key={row.key} className="rounded-lg border border-ash-border bg-ash-bg/60 p-3">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-sm font-medium text-ash-text">{row.label}</div>
                  <div className="text-xs text-ash-muted mt-0.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 text-xs text-ash-text-secondary mt-3">
              ...
              {row.meta.fallback_used && (
                <div className="mt-2 text-xs text-ash-warning bg-ash-warning/10 border border-ash-warning/30 rounded-md px-2 py-1.5">
