...
export function Watchlist({ activeSymbol, onSymbolSelect }: WatchlistProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="p-4 border-b border-ash-border">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-ash-text">自选列表</h2>
        ...
        {isAdding && (
          <div className="flex gap-2">
            ...
            <Button
              ...
              aria-label="取消添加"
              className="p-1.5 text-ash-muted hover:text-ash-text"
            >
              <X size={12} />
            </Button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {watchlist.length === 0 ? (
          <div className="p-4 text-center text-xs text-ash-muted">
            暂无自选，点击上方 + 添加
          </div>
        ) : (
          <ul className="py-2">
              ...
              return (
                <li
                  ...
                  className={`group flex flex-col px-4 py-2.5 cursor-pointer transition-colors ${
                    isActive
                      ? 'bg-ash-primary/10 border-l-2 border-ash-primary'
                      : 'hover:bg-ash-hover border-l-2 border-transparent'
                  }`}
                >
                  {/* 主行：Symbol + 价格 + 操作 */}
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col min-w-0 flex-shrink-0">
                      <span
                        className={`text-sm font-medium truncate ${
                          isActive ? 'text-ash-primary' : 'text-ash-text'
                        }`}
                      >
                        {item.symbol}
                      </span>
                      <span className="text-2xs text-ash-muted truncate max-w-[80px]">
                    ...
                    <div className="flex flex-col items-end flex-1 mx-2">
                      <span className="text-xs font-medium text-ash-text">
                        {hasPrice ? `$${quote.price!.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '--'}
                      </span>
                      <span className={`text-2xs font-medium ${isUp ? 'text-ash-up' : 'text-ash-down'}`}>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                      ...
                      <Button
                        ...
                        className="p-1 rounded hover:bg-ash-bg-secondary text-ash-muted hover:text-ash-primary"
                        title="设置持仓"
                      >
                        <Package size={12} />
                      </Button>
                      ...
                      <Button
                        ...
                        className="p-1 rounded hover:bg-ash-bg-secondary text-ash-muted hover:text-ash-danger"
                        title="删除"
                      >
                        <X size={12} />
                      </Button>
                  ...
                  {shares > 0 && !isEditingThis && (
                    <button
                      ...
                      className="mt-1 text-2xs text-ash-primary bg-ash-primary/10 px-1.5 py-0.5 rounded-full w-fit hover:bg-ash-primary/20 transition-colors"
                    >
                      持仓 {shares} 股
                    </button>
                  )}
                  ...
                  {isEditingThis && (
                    ...
                    >
                      <Package size={10} className="text-ash-muted shrink-0" />
                      <input
                        ...
                        onBlur={saveHoldings}
                        placeholder="股数（0=清除）"
                        className="flex-1 min-w-0 px-1.5 py-0.5 text-2xs rounded border border-ash-primary/50 bg-ash-bg text-ash-text text-right focus:outline-none focus:border-ash-primary"
                      />
                      <Button
                        variant="ghost"
                        size="sm"
                        ...
                        aria-label="确认持仓"
                        className="p-0.5 text-ash-success hover:text-ash-success"
                        title="确认"
                      >
                        <Check size={12} />
                      </Button>
                      <Button
                        ...
                        aria-label="取消编辑"
                        className="p-0.5 text-ash-muted hover:text-ash-danger"
                        title="取消"
                      >
                        <X size={12} />
                      </Button>
      ...
      {contextMenu && (
        <div
          className="fixed z-50 bg-ash-card border border-ash-border rounded-lg shadow-lg py-1 min-w-[140px]"
        ...
        >
          ...
          <Button
            ...
            className="w-full px-3 py-1.5 text-left text-xs text-ash-danger justify-start"
          >
            <X size={12} />
            删除
          </Button>
      ...
      {/* 底部信息 */}
      <div className="p-3 border-t border-ash-border text-center">
