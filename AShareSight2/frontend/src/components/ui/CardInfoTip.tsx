...
export function CardInfoTip({
  ...
  const iconClassName = icon === 'alert'
    ? 'text-ash-warning/70 group-hover/tip:text-ash-warning'
    : 'text-ash-muted/50 group-hover/tip:text-ash-muted';
    ...
    <span className={`relative inline-flex group/tip ${className}`} data-testid={testId}>
      ...
      <span
        ...
        className="pointer-events-none absolute right-0 top-full mt-1.5
          w-max max-w-[220px] whitespace-normal rounded-md border border-ash-border
          bg-ash-panel px-2.5 py-1.5 text-2xs text-ash-text leading-relaxed
