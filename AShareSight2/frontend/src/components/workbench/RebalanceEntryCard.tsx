      ...
      <div className="space-y-3">
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-3 text-ash-text font-semibold text-sm">
            <Scale size={16} className="text-ash-primary" />
            AI 智能调仓
          </div>
          ...
          {loading && (
            <div className="flex items-center gap-2 text-xs text-ash-muted py-2">
              <Loader2 size={14} className="animate-spin text-ash-primary" />
              正在分析持仓并生成建议...
            </div>
          )}
        </Card>
  ...
  if (suggestion && !showDetails) {
    return (
      <Card className="p-4 space-y-3">
        ...
        <p className="text-xs text-ash-text-secondary leading-relaxed line-clamp-3">
          {suggestion.summary}
        </p>
