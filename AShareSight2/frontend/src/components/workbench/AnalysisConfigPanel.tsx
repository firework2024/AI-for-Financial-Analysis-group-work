...
export function AnalysisConfigPanel({
  ...
  return (
    ...
    >
      <Card className="p-5 border border-ash-border bg-ash-card text-ash-text">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h3 id="analysis-config-title" className="text-base font-semibold text-ash-text">分析配置</h3>
            <p className="text-xs text-ash-muted mt-1 break-all">
              {queryPreview || '未输入查询'}
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="text-xs px-2 py-1 rounded border border-ash-border text-ash-muted hover:text-ash-text"
          >
            关闭
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <p className="text-xs text-ash-muted mb-2">分析深度</p>
              ...
              {DEPTHS.map((depth) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs border transition-colors ${
                    analysisDepth === depth
                      ? 'border-ash-primary/60 bg-ash-primary/10 text-ash-primary'
                      : 'border-ash-border text-ash-muted hover:text-ash-text'
                  }`}
                >
                  {DEPTH_LABELS[depth]}
                </button>
          ...
          <div>
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="text-ash-muted">预算轮次</span>
              <span className="text-ash-text">{clampBudget(budget)}</span>
            </div>
            <input
              type="range"
              min={1}
              ...
              className="w-full accent-ash-primary"
            />
          </div>

          <label className="inline-flex items-center gap-2 text-xs text-ash-muted">
            <input
              type="checkbox"
          ...
          <div>
            <p className="text-xs text-ash-muted mb-2">本次执行 Agent 深度覆盖</p>
            <div className="space-y-2 max-h-56 overflow-auto pr-1">
              {Object.entries(AGENT_LABELS).map(([name, label]) => {
                ...
                return (
                  <div
                    key={name}
                    className="flex items-center justify-between gap-3 rounded border border-ash-border px-3 py-2"
                  >
                    <span className="text-xs text-ash-text">{label}</span>
                            ...
                            depth === option
                              ? option === 'off'
                                ...
                                : option === 'deep'
                                  ...
                                  : 'border-ash-primary/60 text-ash-primary bg-ash-primary/10'
                              : 'border-ash-border text-ash-muted hover:text-ash-text'
                          }`}
                        >
          ...
          <div className="rounded border border-ash-border px-3 py-2">
            <p className="text-xs text-ash-muted mb-1">当前可用工具</p>
            {loadingCapabilities ? (
              <p className="text-xs text-ash-muted">加载中...</p>
            ) : (
              <div className="flex flex-wrap gap-1">
                {selectedTools.length > 0 ? (
                  selectedTools.map((tool) => (
                    <span
                      key={tool}
                      className="text-2xs px-1.5 py-0.5 rounded bg-ash-primary/10 text-ash-primary border border-ash-primary/30"
                    >
                      {tool}
                    </span>
                  ))
                ) : (
                  <span className="text-xs text-ash-muted">暂无可用工具</span>
                )}
              </div>
            )}
          </div>
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-3 py-1.5 rounded text-xs border border-ash-border text-ash-muted hover:text-ash-text"
          >
            取消
          </button>
          <button
            ...
            className="px-3 py-1.5 rounded text-xs bg-ash-primary/15 text-ash-primary border border-ash-primary/40 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            以此配置开始
          </button>
        </div>
