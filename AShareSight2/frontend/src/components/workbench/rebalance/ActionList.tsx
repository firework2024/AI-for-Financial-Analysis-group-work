...
const ACTION_STYLES: Record<ActionType, ActionStyle> = {
  ...
  hold: { label: '持有', textClass: 'text-ash-muted', bgClass: 'bg-ash-bg-secondary' },
};
...
export function ActionList({ actions, decisions, onSetDecision }: ActionListProps) {
  ...
  if (actions.length === 0) {
    return (
      <p className="text-sm text-ash-muted py-4 text-center">
        暂无调仓操作建议
      </p>
    );
  }
...
function ActionRow({
  ...
  return (
    <>
      <tr
        className={`border-b border-ash-border/50 hover:bg-ash-hover/50 cursor-pointer transition-all ${rowOpacity}`}
        tabIndex={0}
        role="button"
      ...
      >
        <td className="py-2 px-2">
          {isExpanded
            ? <ChevronDown size={12} className="text-ash-muted" />
            : <ChevronRight size={12} className="text-ash-muted" />}
        </td>
        <td className="py-2 px-2 font-semibold text-ash-text">{action.ticker}</td>
        <td className="py-2 px-2">
          <Badge className={`${style.bgClass} ${style.textClass}`}>
            {style.label}
          </Badge>
        </td>
        <td className="py-2 px-2 text-right text-ash-text-secondary">
          {formatWeight(action.current_weight)}
        </td>
        <td className="py-2 px-1 text-center text-ash-muted">
          {'\u2192'}
        </td>
        <td className="py-2 px-2 text-right text-ash-text">
        ...
        <td className="py-2 px-2 text-center">
          <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-ash-bg-secondary text-ash-text text-2xs font-medium">
            {action.priority}
          </span>
        </td>
        {hasWorkflow && (
      ...
      {/* 展开详情行 */}
      {isExpanded && (
        <tr className="bg-ash-bg-secondary/50">
          <td colSpan={colSpanCount} className="px-4 py-3">
            <div className="space-y-2">
              <div>
                <span className="text-2xs text-ash-muted font-medium">调仓理由</span>
                <p className="text-xs text-ash-text-secondary mt-0.5 leading-relaxed">
                  {action.reason}
                </p>
              </div>
              <div>
                <span className="text-2xs text-ash-muted font-medium">引用来源</span>
    ...
    <div className="inline-flex items-center gap-1">
      <button
        ...
        className={`p-1 rounded transition-colors ${
          decision === 'accepted'
            ...
            : 'text-ash-muted hover:text-emerald-500 hover:bg-emerald-500/10'
        }`}
      >
        <Check size={12} />
      </button>
      <button
        ...
        className={`p-1 rounded transition-colors ${
          decision === 'rejected'
            ...
            : 'text-ash-muted hover:text-red-400 hover:bg-red-500/10'
        }`}
      >
        <X size={12} />
      </button>
      {decision !== 'pending' && (
        <button
          ...
          onClick={handleReset}
          title="重置"
          className="p-1 rounded text-ash-muted hover:text-ash-text hover:bg-ash-hover transition-colors"
        >
          <RotateCcw size={10} />
        </button>
      )}
