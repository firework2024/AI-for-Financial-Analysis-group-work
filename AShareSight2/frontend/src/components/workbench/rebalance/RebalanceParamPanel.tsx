  ...
  return (
    <Card id="rebalance-param-panel" className="p-4 space-y-4">
      <div className="flex items-center gap-2 text-ash-text font-semibold text-sm">
        <Settings2 size={16} className="text-ash-primary" />
        调仓参数配置
      </div>

      {/* Risk tier radio group */}
      <div className="space-y-2">
        <span className="text-xs text-ash-muted font-medium">风险偏好</span>
            ...
            <button
              ...
              className={`
                ...
                ${riskTier === tier.value
                  ...
                  : 'border-ash-border bg-ash-card text-ash-text-secondary hover:bg-ash-hover'}
              `.trim()}
            >
              <div className="font-medium">{tier.label}</div>
              <div className="text-2xs text-ash-muted mt-0.5">{tier.desc}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none">
        <input
          ...
          className="accent-ash-primary w-3.5 h-3.5"
        />
        <Sparkles size={14} className="text-ash-primary" />
        <span className="text-xs text-ash-text">AI 增强分析</span>
        <span className="text-2xs text-ash-muted">(使用 LLM 优化建议)</span>
      </label>
      ...
      <div>
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-ash-text-secondary hover:text-ash-text transition-colors"
        >
      ...
      {portfolio.length === 0 && (
        <p className="text-2xs text-ash-warning text-center">
          请先添加持仓后再生成建议
        </p>
      )}
    </Card>
...
function ConstraintInput({ label, value, onChange }: ConstraintInputProps) {
  return (
    <div className="flex items-center justify-between gap-3">
      <span className="text-2xs text-ash-muted whitespace-nowrap">{label}</span>
      <input
        type="number"
        min={0}
        max={100}
        ...
        className="w-16 px-2 py-1 text-xs text-ash-text bg-ash-bg-secondary border border-ash-border rounded-md text-right focus:outline-none focus:ring-1 focus:ring-ash-primary"
      />
    </div>
  );
}
