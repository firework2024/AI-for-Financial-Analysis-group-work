  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start gap-2">
        <Brain size={14} className="shrink-0 mt-0.5 text-ash-primary" />
        <p className="text-sm text-ash-text leading-relaxed">{suggestion.summary}</p>
      </div>
...
function ImpactMetric({ icon, label, value }: ImpactMetricProps) {
  return (
    <div className="flex flex-col items-center gap-1 px-2 py-2 rounded-lg bg-ash-bg-secondary">
      <span className="text-ash-muted">{icon}</span>
      <span className="text-2xs text-ash-muted">{label}</span>
      <span className="text-xs font-semibold text-ash-text">{value}</span>
    </div>
  );
}
