...
export function EvidenceLinks({ snapshots }: EvidenceLinksProps) {
  if (snapshots.length === 0) {
    return (
      <p className="text-2xs text-ash-muted italic py-1">
        暂无引用来源
      </p>
    );
  }
    ...
    <ul className="space-y-1.5">
      {snapshots.map((snap) => (
        <li
          ...
          className="flex items-start gap-2 text-2xs text-ash-text-secondary"
        >
          <FileText size={12} className="shrink-0 mt-0.5 text-ash-muted" />
          <div className="min-w-0">
            <span className="font-medium text-ash-text">{snap.source}</span>
            <span className="mx-1 text-ash-muted">--</span>
            <span className="text-ash-text-secondary">{truncateQuote(snap.quote)}</span>
          </div>
        </li>
      ))}
    </ul>
