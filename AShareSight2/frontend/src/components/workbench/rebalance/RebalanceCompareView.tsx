  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-ash-card/50 rounded-lg border border-ash-border/50 p-2">
          <ReactECharts
            option={beforeOption}
      ...
      {/* 统计摘要 */}
      <div className="text-center text-2xs text-ash-muted">
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {diffRows.map((row) => (
              <tr
                key={row.ticker}
                className={`border-b border-ash-border/30 ${
                  row.accepted ? '' : 'opacity-50'
                }`}
              >
                <td className="py-1.5 px-2 font-semibold text-ash-text">
                  {row.ticker}
                </td>
                <td className="py-1.5 px-2 text-right text-ash-text-secondary">
                  {row.before.toFixed(1)}%
                </td>
                <td className="py-1.5 px-2 text-right text-ash-text">
                  {row.after.toFixed(1)}%
                </td>
                <td
                ...
                <td className="py-1.5 px-2 text-center">
                  <span
                    className={`inline-block w-2 h-2 rounded-full ${
                      row.accepted ? 'bg-emerald-500' : 'bg-ash-muted/40'
                    }`}
                  />
                </td>
              </tr>
