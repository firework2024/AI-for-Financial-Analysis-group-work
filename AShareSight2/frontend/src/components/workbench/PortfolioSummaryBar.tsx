...
export function PortfolioSummaryBar() {
  ...
  return (
    <div className="flex items-center gap-6 px-4 py-2.5 bg-ash-card border border-ash-border rounded-xl overflow-x-auto scrollbar-hide">
      {metrics.map((metric) => (
        <div key={metric.label} className="flex items-center gap-2 whitespace-nowrap">
          <span className="text-ash-text-secondary">{metric.icon}</span>
          <span className="text-xs text-ash-muted">{metric.label}</span>
          {loading ? (
            <Skeleton variant="rectangular" className="w-20 h-3" />
          ) : (
            <span className={`text-xs font-semibold ${metric.color ?? 'text-ash-text'}`}>{metric.value}</span>
          )}
        </div>
      ))}
    </div>
