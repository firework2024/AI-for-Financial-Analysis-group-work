    ...
    <div className="space-y-2">
      {Array.from({ length: 3 }).map((_, i) => (
        <div
          ...
          className="p-2 rounded-lg border border-ash-border animate-pulse"
        >
          <div className="h-3.5 bg-ash-bg-secondary rounded w-3/4" />
          <div className="h-2.5 bg-ash-bg-secondary rounded w-1/2 mt-2" />
          <div className="flex gap-1 mt-2">
            <div className="h-4 w-10 bg-ash-bg-secondary rounded" />
            <div className="h-4 w-12 bg-ash-bg-secondary rounded" />
          </div>
        </div>
      ))}
    </div>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-6 text-ash-muted gap-2">
  ...
  return (
    <div>
      <button
        ...
        className="flex items-center gap-1.5 w-full text-left py-1 text-xs font-medium text-ash-text-secondary hover:text-ash-text transition-colors"
      >
        {expanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
        <span>{group.label}</span>
        <span className="text-ash-muted">({group.items.length})</span>
      </button>

      {expanded && (
        <div className="ml-2 border-l border-ash-border pl-3 space-y-1.5 mt-1">
            ...
            return (
              <div
                ...
                className={`p-2 rounded-lg border transition-colors ${
                  isSelected
                    ? 'border-ash-primary/60 bg-ash-primary/5'
                    : isPreviewActive
                      ? 'border-ash-primary/40 bg-ash-primary/5'
                      : 'border-ash-border hover:border-ash-primary/30 hover:bg-ash-bg'
                }`}
              >
                <div className="flex items-start gap-2">
                  ...
                  {compareMode && (
                    <label className="flex items-center mt-0.5 cursor-pointer shrink-0">
                      <input
                        ...
                        className="w-3.5 h-3.5 rounded border-ash-border text-ash-primary focus:ring-ash-primary/30 cursor-pointer disabled:opacity-40"
                      />
                    </label>
                  )}
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs text-ash-text font-medium truncate flex-1">
                        {item.title || item.report_id}
                      </span>
                      <ConfidenceBadge score={item.confidence_score} />
                    </div>

                    <div className="text-2xs text-ash-muted mt-0.5 flex items-center gap-1.5">
                      {item.ticker && (
                        <span className="font-medium text-ash-text-secondary">{item.ticker}</span>
                      )}
  ...
  return (
    <Card className="p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 text-ash-text font-semibold text-sm">
          <Library size={16} className="text-ash-primary" />
          研报时间线
        </div>
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2 py-1 rounded text-2xs transition-colors ${
            compareMode
              ? 'bg-ash-primary/10 text-ash-primary'
              : 'text-ash-muted hover:text-ash-text hover:bg-ash-hover'
          }`}
        >
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="relative flex-1">
          <Search
            size={14}
            className="absolute left-2.5 top-1/2 -translate-y-1/2 text-ash-muted pointer-events-none"
          />
          <Input
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none bg-ash-bg border border-ash-border rounded-lg px-2.5 py-1.5 pr-7 text-xs text-ash-text cursor-pointer focus:outline-none focus:border-ash-primary focus:ring-1 focus:ring-ash-primary/30 transition-colors"
          >
          ...
          <ChevronDown
            size={12}
            className="absolute right-2 top-1/2 -translate-y-1/2 text-ash-muted pointer-events-none"
          />
        </div>
      </div>
      ...
      {/* Compare action bar */}
      {compareMode && (
        <div className="flex items-center gap-2 mb-3 px-2 py-1.5 rounded-lg bg-ash-bg-secondary text-2xs text-ash-muted">
          <span>已选 {compareIds.size}/2 份报告</span>
          {compareIds.size === 2 && (
            <button
              type="button"
              onClick={() => setShowCompare(true)}
              className="ml-auto px-2 py-0.5 rounded bg-ash-primary text-white text-2xs hover:bg-ash-primary/90 transition-colors"
            >
              开始对比
            </button>
          )}
