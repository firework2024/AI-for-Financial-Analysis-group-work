...
export function QuickAnalysisBar({ defaultTicker, onAnalysisStarted }: QuickAnalysisBarProps) {
  ...
  return (
    <>
      <div className="flex items-center gap-2 px-3 py-2 bg-ash-card rounded-xl border border-ash-border hover:border-ash-primary/30 transition-colors">
        <Search className="w-4 h-4 text-ash-muted flex-shrink-0" />
        <input
          ref={inputRef}
          type="text"
          value={query}
          ...
          className="flex-1 bg-transparent text-sm text-ash-text placeholder:text-ash-muted/60 outline-none"
          disabled={isRunning}
        />
        <button
          type="button"
          ...
          disabled={!canSubmit}
          className="flex items-center gap-1 px-2 py-1 text-xs rounded-lg border border-ash-border text-ash-muted hover:text-ash-text disabled:opacity-40 disabled:cursor-not-allowed"
          title="执行前配置"
        >
        ...
        <button
          ...
          disabled={!canSubmit}
          className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-lg bg-ash-primary/10 text-ash-primary hover:bg-ash-primary/20 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          {isRunning ? (
