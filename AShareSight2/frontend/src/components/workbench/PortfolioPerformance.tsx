...
function pnlColor(value: number | null | undefined): string {
  if (value === null || value === undefined || value === 0) return 'text-ash-muted';
...
function HeaderCell({
  ...
  return (
    <th
      className={`px-3 py-2 text-2xs font-medium text-ash-muted whitespace-nowrap cursor-pointer select-none hover:text-ash-text transition-colors ${
    ...
    >
      <span className="inline-flex items-center gap-1">
        ...
        <ArrowUpDown
          size={10}
          className={isActive ? 'text-ash-primary' : 'opacity-30'}
        />
        {isActive && (
          <span className="text-ash-primary text-[9px]">
            {activeSortDir === 'asc' ? '\u25B2' : '\u25BC'}
          </span>
        )}
      </span>
...
function SummaryFooter({ summary }: { summary: PerformanceSummary }) {
  return (
    <tr className="border-t border-ash-border bg-ash-card/80">
      <td className="px-3 py-2.5 text-xs font-semibold text-ash-text">
      ...
      {/* total market value */}
      <td className="px-3 py-2.5 text-xs font-semibold text-ash-text text-right">
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-8 gap-2">
      <TrendingUp size={28} className="text-ash-muted/40" />
      <div className="text-xs text-ash-muted">暂无持仓数据，请先添加持仓</div>
    </div>
  );
}
...
export function PortfolioPerformance({ data, loading }: PortfolioPerformanceProps) {
  ...
  return (
    <div className="bg-ash-card border border-ash-border rounded-xl overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-ash-border">
        <div className="flex items-center gap-2">
          <TrendingUp size={14} className="text-ash-primary" />
          <span className="text-xs font-semibold text-ash-text">持仓收益追踪</span>
        </div>
        {!isEmpty && (
          <div className="flex items-center gap-3 text-2xs text-ash-muted">
            <span>
              总收益：
      ...
      {!loading && !isEmpty && (
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-ash-border/60">
            ...
            <tbody>
              {sorted.map((row) => (
                <tr
                  key={row.ticker}
                  className="border-b border-ash-border/30 hover:bg-ash-hover/40 transition-colors"
                >
                  <td className="px-3 py-2 text-xs font-medium text-ash-text whitespace-nowrap">
                    {row.ticker}
                    <span className="ml-1.5 text-2xs text-ash-muted">
                      x{row.shares}
                    </span>
                  </td>
                  <td className="px-3 py-2 text-xs text-ash-text text-right tabular-nums">
