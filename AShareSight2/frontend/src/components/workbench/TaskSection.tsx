...
function TaskSection({ symbol }: TaskSectionProps) {
  ...
  return (
    <Card className="p-4">
      <div className="flex items-center gap-2 mb-3 text-ash-text font-semibold text-sm">
        <ListTodo size={16} className="text-ash-primary" />
        今日任务
      </div>

      {isLoading && (
        <div className="flex items-center gap-2 text-xs text-ash-muted py-2">
      ...
      {fetchError && !isLoading && (
        <div className="text-xs text-ash-danger py-2">加载失败: {fetchError}</div>
      )}

      <div className="space-y-1.5">
        {!isLoading && tasks.length === 0 && !fetchError && (
          <div className="text-xs text-ash-muted py-2">暂无建议任务</div>
        )}
      ...
      {recentHistory.length > 0 && (
        <div className="mt-4 pt-3 border-t border-ash-border">
          <div className="text-2xs font-medium text-ash-text-secondary mb-2">执行历史</div>
          <div className="space-y-1">
            {recentHistory.map((item, idx) => (
              <div key={`${item.taskId}-${item.at}-${String(idx)}`} className="text-2xs text-ash-muted flex items-center gap-2">
