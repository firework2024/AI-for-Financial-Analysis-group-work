...
function DeltaIndicator({ value }: { value: number | null }) {
  if (value === null || value === 0) {
    return <Minus size={12} className="text-ash-muted" />;
  }
  if (value > 0) {
    return (
      <span className="inline-flex items-center gap-0.5 text-ash-success text-2xs font-medium">
        <ArrowUp size={10} />
        +{(value * 100).toFixed(1)}%
      </span>
    );
  ...
  return (
    <span className="inline-flex items-center gap-0.5 text-ash-danger text-2xs font-medium">
      <ArrowDown size={10} />
      {(value * 100).toFixed(1)}%
    </span>
  );
...
function SentimentBadge({ sentiment }: { sentiment: string | null }) {
  if (!sentiment) return <span className="text-ash-muted text-2xs">--</span>;
...
function ScoreDisplay({ score, label }: { score: number | null; label: string }) {
  ...
  return (
    <div className="text-center">
      <div className="text-2xs text-ash-muted mb-1">{label}</div>
      <div
...
function DiffSummary({ textA, textB }: { textA: string; textB: string }) {
  ...
  const renderHighlighted = (text: string, otherWords: Set<string>) => {
    if (!text) return <span className="text-ash-muted">暂无摘要</span>;
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <div>
        <div className="text-2xs text-ash-muted mb-1">报告 A</div>
        <div className="text-2xs text-ash-text leading-relaxed line-clamp-5">
          {renderHighlighted(textA, wordsB)}
        </div>
      </div>
      <div>
        <div className="text-2xs text-ash-muted mb-1">报告 B</div>
        <div className="text-2xs text-ash-text leading-relaxed line-clamp-5">
    ...
    <Card className="p-4">
      ...
      <div className="flex items-center gap-2 mb-4">
        <button
          type="button"
          onClick={onClose}
          className="p-1 rounded hover:bg-ash-hover text-ash-muted hover:text-ash-text transition-colors"
          aria-label="返回"
        >
          <ArrowLeft size={16} />
        </button>
        <span className="text-sm font-semibold text-ash-text">报告对比</span>
      </div>

      {/* Loading */}
      {loading && (
        <div className="flex items-center justify-center gap-2 py-8 text-ash-muted text-xs">
      ...
      {diff && !loading && (
        <div className="space-y-4">
          {/* Report meta headers */}
          <div className="grid grid-cols-2 gap-3">
            <div className="p-2 rounded-lg bg-ash-bg-secondary">
              <div className="text-2xs text-ash-muted mb-0.5 font-medium">报告 A</div>
              <div className="text-xs text-ash-text font-medium truncate" title={labelA}>
                {labelA}
              </div>
              {reportA?.generated_at && (
                <div className="flex items-center gap-1 text-2xs text-ash-muted mt-0.5">
                  <Calendar size={10} />
            ...
            <div className="p-2 rounded-lg bg-ash-bg-secondary">
              <div className="text-2xs text-ash-muted mb-0.5 font-medium">报告 B</div>
              <div className="text-xs text-ash-text font-medium truncate" title={labelB}>
                {labelB}
              </div>
          ...
          {/* Confidence Score */}
          <div className="p-3 rounded-lg bg-ash-bg-secondary">
            <div className="text-2xs text-ash-muted mb-2 font-medium">置信度对比</div>
            <div className="grid grid-cols-3 items-center">
              <ScoreDisplay score={diff.confidence_score.a} label="报告 A" />
              <div className="flex flex-col items-center gap-1">
                <div className="text-2xs text-ash-muted">变化</div>
          ...
          {/* Sentiment */}
          <div className="p-3 rounded-lg bg-ash-bg-secondary">
            <div className="text-2xs text-ash-muted mb-2 font-medium">情绪判断</div>
          ...
          {/* Risks */}
          <div className="p-3 rounded-lg bg-ash-bg-secondary">
            <div className="text-2xs text-ash-muted mb-2 font-medium">风险变化</div>
            <div className="text-2xs text-ash-muted mb-2">
              不变风险: {diff.risks.unchanged_count} 个
            </div>

            {diff.risks.added.length > 0 && (
              <div className="mb-2">
                <div className="flex items-center gap-1 text-2xs text-ash-danger mb-1">
                  <ShieldAlert size={10} />
                  新增风险 ({diff.risks.added.length})
                </div>
                <ul className="space-y-0.5">
                  {diff.risks.added.map((risk, i) => (
                    <li key={`added-${String(i)}`} className="text-2xs text-ash-text pl-3 truncate">
                      • {risk}
                    </li>
                  ))}
                </ul>
            ...
            {diff.risks.removed.length > 0 && (
              <div>
                <div className="flex items-center gap-1 text-2xs text-ash-success mb-1">
                  <ShieldCheck size={10} />
                  已消除风险 ({diff.risks.removed.length})
                </div>
                <ul className="space-y-0.5">
                  {diff.risks.removed.map((risk, i) => (
                    <li key={`removed-${String(i)}`} className="text-2xs text-ash-text pl-3 truncate">
                      • {risk}
                    </li>
                  ))}
                </ul>
            ...
            {diff.risks.added.length === 0 && diff.risks.removed.length === 0 && (
              <div className="text-2xs text-ash-muted">风险评估无变化</div>
            )}
          </div>

          {/* Summary comparison with diff highlight */}
          <div className="p-3 rounded-lg bg-ash-bg-secondary">
            <div className="text-2xs text-ash-muted mb-2 font-medium">摘要对比</div>
            <DiffSummary
