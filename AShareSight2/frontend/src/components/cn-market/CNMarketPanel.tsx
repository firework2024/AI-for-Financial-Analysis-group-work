...
function pctColor(val: unknown): string {
  const n = Number(val);
  if (!Number.isFinite(n)) return 'text-ash-muted';
  return n > 0 ? 'text-ash-up' : n < 0 ? 'text-ash-down' : 'text-ash-muted';
}
...
function FundFlowTable({ rows }: { rows: RowData[] }) {
  return (
    <table className="w-full text-xs">
      <thead>
        <tr className="text-ash-muted border-b border-ash-border">
      ...
      <tbody className="divide-y divide-ash-border/30">
        {rows.map((row, i) => (
          <tr key={String(row.symbol ?? i)} className="hover:bg-ash-hover/30 transition-colors">
            <td className="py-1.5 font-mono text-ash-muted">{String(row.symbol ?? '-')}</td>
            <td className="py-1.5 text-ash-text max-w-[80px] truncate">{String(row.name ?? '-')}</td>
      ...
      <tbody className="divide-y divide-ash-border/30">
        {rows.map((row, i) => (
          <tr key={String(row.symbol ?? i)} className="hover:bg-ash-hover/30 transition-colors">
            ...
            <td className="py-1.5 text-ash-text max-w-[80px] truncate">{String(row.name ?? '-')}</td>
            <td className={`py-1.5 text-right ${pctColor(row.change_percent)}`}>{fmtPct(row.change_percent)}</td>
            <td className="py-1.5 text-right text-ash-text">{String(row.turnover_rate ?? '-')}</td>
            <td className="py-1.5 text-right text-ash-text">{String(row.volume_ratio ?? '-')}</td>
          </tr>
        ))}
      </tbody>
    </table>
    ...
    <table className="w-full text-xs">
      ...
      <tbody className="divide-y divide-ash-border/30">
        {rows.map((row, i) => (
          <tr key={String(row.symbol ?? i)} className="hover:bg-ash-hover/30 transition-colors">
            <td className="py-1.5 font-mono text-ash-muted">{String(row.symbol ?? '-')}</td>
            <td className="py-1.5 text-ash-text max-w-[70px] truncate">{String(row.name ?? '-')}</td>
            <td className={`py-1.5 text-right ${pctColor(row.change_percent)}`}>{fmtPct(row.change_percent)}</td>
            <td className={`py-1.5 text-right ${flowColor(row.net_buy)}`}>{fmtFlow(row.net_buy)}</td>
            <td className="py-1.5 pl-2 text-ash-muted max-w-[100px] truncate" title={String(row.reason ?? '')}>
              {String(row.reason ?? '-')}
            </td>
          </tr>
        ))}
...
function ConceptTable({ rows }: { rows: RowData[] }) {
  return (
    <table className="w-full text-xs">
      ...
      <tbody className="divide-y divide-ash-border/30">
        {rows.map((row, i) => (
          <tr key={String(row.concept_code ?? i)} className="hover:bg-ash-hover/30 transition-colors">
            <td className="py-1.5 text-ash-text max-w-[110px] truncate">{String(row.concept_name ?? '-')}</td>
            <td className={`py-1.5 text-right ${pctColor(row.change_percent)}`}>{fmtPct(row.change_percent)}</td>
            <td className={`py-1.5 text-right ${flowColor(row.main_net_inflow)}`}>{fmtFlow(row.main_net_inflow)}</td>
            <td className="py-1.5 text-right">
              <span className="text-ash-up">{String(row.up_count ?? '-')}</span>
              <span className="text-ash-muted mx-0.5">/</span>
              <span className="text-ash-down">{String(row.down_count ?? '-')}</span>
            </td>
          </tr>
        ))}
      </tbody>
...
export const CNMarketPanel: React.FC = () => {
  ...
  return (
    <section className="rounded-xl border border-ash-border bg-ash-card p-4">
      {/* Header */}
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ash-text">A股市场</h3>
        <button
          type="button"
          onClick={() => loadTab(activeTab)}
          disabled={isLoading}
          className="p-1.5 rounded-lg text-ash-muted hover:text-ash-text hover:bg-ash-hover transition-colors disabled:opacity-40"
          title="刷新"
        >
      ...
      {/* Tab bar */}
      <div className="flex gap-1 mb-3 border-b border-ash-border pb-2 overflow-x-auto">
      ...
      {/* Error */}
      {error && <p className="mb-2 text-xs text-ash-danger">{error}</p>}

      {/* Loading skeleton */}
      {isLoading && rows.length === 0 && (
        <div className="space-y-1.5">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-7 rounded bg-ash-border/30 animate-pulse" />
          ))}
        </div>
      )}
      ...
      {!isLoading && !error && rows.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 text-ash-muted gap-2">
          <p className="text-xs">暂无数据，点击立即加载</p>
          <button
            type="button"
            data-testid="cn-market-load-btn"
            onClick={() => loadTab(activeTab)}
            className="text-xs text-ash-primary hover:underline"
          >
            立即加载
          </button>
        </div>
