...
export const RightPanel: FC<RightPanelProps> = ({
  ...
  return (
    <section
      ...
      className={`flex flex-col h-full bg-ash-card border border-ash-border rounded-xl shadow-sm overflow-hidden ${className || ''}`}
    >
      <RightPanelHeader
        activeTab={activeTab}
      ...
      {showMiniChat && (
        <div className="h-[45%] min-h-[180px] border-t border-ash-border flex flex-col">
          <MiniChat />
        </div>
      )}

      {lastUpdated && (
        <div className="text-2xs text-ash-muted text-center py-1 border-t border-ash-border/50 shrink-0">
