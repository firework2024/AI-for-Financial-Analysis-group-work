...
function statusClasses(status: AgentRunInfo['status']): { icon: ReactNode; text: string; bar: string } {
  switch (status) {
    ...
    case 'skipped':
      return {
        icon: <SkipForward size={13} className="text-ash-muted" />,
        text: 'text-ash-muted bg-ash-bg/40',
        bar: 'bg-ash-muted',
      };
    default:
      return {
        icon: <Clock3 size={13} className="text-ash-muted" />,
        text: 'text-ash-muted bg-ash-bg/40',
        bar: 'bg-ash-border',
      };
  }
  ...
  return (
    <section className={`space-y-2 ${className}`} aria-label="Agent 执行进度">
      <div className="flex items-center justify-between gap-2 px-1 text-2xs">
        <div className="text-ash-text-secondary">
          Agent 进度
          <span className="ml-1 text-ash-muted">({doneCount}/{agents.length})</span>
        </div>
        {activeCount > 0 && (
      ...
      <div className="divide-y divide-ash-border/50 border-y border-ash-border/50">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <div className="truncate text-xs font-medium text-ash-text">{displayName}</div>
                      <div
                        className={`mt-0.5 truncate text-2xs ${
                          info.status === 'error' ? 'text-red-300' : 'text-ash-muted'
                        }`}
                        title={detail}
                      >
                        {detail}
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                      {info.parallelGroup && (
                        <span className="rounded px-1.5 py-0.5 text-[10px] leading-[14px] text-ash-warning bg-amber-500/10">
                          {info.parallelGroup}
                        </span>
                      )}
                      {duration && <span className="text-[10px] leading-[14px] text-ash-muted">{duration}</span>}
                  ...
                  <div
                    className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-ash-bg/60"
                    role="progressbar"
                    aria-valuemin={0}
                    aria-valuemax={100}
