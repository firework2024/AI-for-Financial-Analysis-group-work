...
export function AgentTimeline({
  ...
  return (
    <div className="border border-ash-border rounded-lg bg-ash-bg/40">
      <button
        type="button"
        onClick={() => setExpanded((prev) => !prev)}
        className="w-full flex items-center justify-between px-3 py-2 text-xs text-ash-text-secondary hover:text-ash-text transition-colors"
      >
      ...
      {expanded && (
        <div className={`${compact ? 'max-h-40' : 'max-h-64'} overflow-y-auto border-t border-ash-border/60`}>
          <ul className="divide-y divide-ash-border/40">
            {visibleEvents.map((event) => (
              <li key={event.id} className="px-3 py-2 text-2xs">
                <div className="flex items-center justify-between gap-2">
                  <span className={`${isTimelineError(event) ? 'text-red-400' : 'text-ash-text-secondary'}`}>
                    {summarizeTimelineEvent(event)}
                  </span>
                  <span className="text-ash-muted shrink-0">
                    {formatTimelineTime(event.timestamp)}
                  </span>
                </div>
                {(event.agent || event.tool || event.durationMs !== undefined) && (
                  <div className="mt-1 text-ash-muted flex items-center gap-2">
