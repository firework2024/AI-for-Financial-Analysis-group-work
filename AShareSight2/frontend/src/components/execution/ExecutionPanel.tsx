...
function CollapsibleDetails({ details }: { details: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-1 text-ash-muted font-mono text-3xs whitespace-pre-wrap">
      详情：{display}
      {needsTruncation && (
        <button
          type="button"
...
function renderPlanSummary(run: ExecutionRun) {
  ...
  return (
    <div className="rounded-lg border border-ash-border bg-ash-bg/20 px-3 py-2 text-2xs">
      <div className="text-ash-text font-medium">计划摘要</div>
      <div className="mt-1 text-ash-muted flex flex-wrap items-center gap-3">
        <span>步骤：{steps}</span>
        <span>已选 Agent：{selected}</span>
        <span>跳过 Agent：{skipped}</span>
        {run.hasParallelPlan && <span className="text-ash-warning">并行执行</span>}
      </div>
      {run.reasoningBrief && (
        <div className="mt-2 text-ash-text/80 leading-relaxed">{run.reasoningBrief}</div>
      )}
    </div>
  );
}
...
function renderDecisionNotes(run: ExecutionRun) {
  ...
  return (
    <div className="rounded-lg border border-ash-border bg-ash-bg/20">
      <div className="px-3 py-2 border-b border-ash-border/60 text-xs text-ash-text-secondary">
        决策说明
      </div>
      <div className="max-h-52 overflow-y-auto divide-y divide-ash-border/40">
        {notes.slice(-8).reverse().map((note) => (
          <div key={note.id} className="px-3 py-2 text-2xs">
            <div className="text-ash-text font-medium">
              {note.title}
              {note.code && <span className="ml-1.5 text-ash-muted font-mono text-3xs">[{note.code}]</span>}
            </div>
            {note.reason && <div className="mt-1 text-ash-muted">原因：{note.reason}</div>}
            {note.details && Object.keys(note.details).length > 0 && (
              <CollapsibleDetails details={note.details} />
            )}
            {note.impact && <div className="mt-1 text-ash-muted">影响：{note.impact}</div>}
            {note.nextStep && <div className="mt-1 text-ash-muted">下一步：{note.nextStep}</div>}
          </div>
        ))}
      </div>
    </div>
...
export function ExecutionPanel({
  ...
  return (
    <div className={`rounded-xl border border-ash-border bg-ash-card px-3 py-3 space-y-3 ${className}`}>
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <div className="text-2xs text-ash-muted truncate">
          ...
          {collapsible && onCollapse && (
            <button
              type="button"
              onClick={onCollapse}
              className="p-1 rounded hover:bg-ash-hover transition-colors text-ash-muted hover:text-ash-text shrink-0"
              title="收起面板"
            >
              <ChevronDown size={14} />
            </button>
      ...
      {/* ===== 通用状态栏（两种模式共用） ===== */}
      <div className="rounded-lg border border-ash-border bg-ash-bg/20 px-3 py-2 text-xs text-ash-text/90">
        <div>{run.currentStep || '等待执行事件...'}</div>
        {run.status === 'running' && typeof run.etaSeconds === 'number' && run.etaSeconds > 0 && (
          <div className="mt-1 text-2xs text-ash-warning">预计剩余 ~{run.etaSeconds}s</div>
        )}
      </div>
