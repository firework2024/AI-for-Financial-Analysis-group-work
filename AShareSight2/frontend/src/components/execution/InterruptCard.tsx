...
export function InterruptCard({ data, onResume, onCancel }: InterruptCardProps) {
  ...
  return (
    <div className="bg-ash-card border border-ash-border rounded-xl p-5 space-y-4">
      ...
      {data.plan_summary && (
        <div className="text-xs text-ash-text-secondary bg-ash-bg rounded-lg p-3">
          <p className="font-medium text-ash-text mb-1">执行计划</p>
      ...
      {data.required_agents && data.required_agents.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {data.required_agents.map((agent) => (
            <span
              key={agent}
              className="px-2 py-0.5 text-xs rounded-full bg-ash-bg border border-ash-border text-ash-text-secondary"
            >
              {agent}
            </span>
          ))}
                ...
                className={`w-full text-left px-4 py-2 text-xs font-medium rounded-lg border transition-colors disabled:opacity-50 ${
                  isConfirm
                    ...
                    : isCancel
                      ...
                      : 'bg-ash-bg border-ash-border text-ash-text hover:bg-ash-hover'
                }`}
              >
                ...
                {option}
                {isAdjust && (
                  <span className="ml-2 text-2xs text-ash-muted">
                    {showAdjustTemplates ? '（已展开模板）' : '（点此展开模板）'}
                  </span>
                )}
              </button>
              <div className="px-1 text-2xs text-ash-muted">{effect}</div>
            </div>
          );
        })}
      </div>
      ...
      {showAdjustTemplates && (
        <div className="rounded-lg border border-ash-border bg-ash-bg/40 p-3 space-y-2">
          <div className="text-xs font-medium text-ash-text">
            快速模板（点一下就继续执行）
          </div>
              ...
              <button
                ...
                className={`w-full text-left rounded-lg border px-3 py-2 transition-colors disabled:opacity-50 ${
                  selectedTemplateId === template.id
                    ? 'border-ash-primary/60 bg-ash-primary/10'
                    : 'border-ash-border bg-ash-card hover:bg-ash-hover'
                }`}
              >
                <div className="text-xs font-medium text-ash-text">{template.title}</div>
                <div className="mt-0.5 text-2xs text-ash-muted">{template.description}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex gap-2">
        <input
          ...
          disabled={submitting}
          className="flex-1 px-3 py-1.5 text-xs bg-ash-bg border border-ash-border rounded-lg text-ash-text placeholder:text-ash-muted focus:outline-none focus:border-ash-primary disabled:opacity-50"
        />
        <button
          type="button"
          onClick={handleCustomSubmit}
          disabled={submitting || !customInput.trim()}
          className="px-3 py-1.5 text-xs font-medium rounded-lg bg-ash-bg border border-ash-border text-ash-text hover:bg-ash-hover disabled:opacity-50"
        >
          发送
        </button>
      </div>
