...
export function ExecutionStats({ run }: ExecutionStatsProps) {
  ...
  return (
    <div className="rounded-lg border border-ash-border bg-ash-bg/20 px-3 py-2">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-2xs">
        <div className="text-ash-muted">
          LLM 调用
          <div className="text-ash-text text-xs mt-0.5">{stats.llmCalls}</div>
        </div>
        <div className="text-ash-muted">
          工具调用
          <div className="text-ash-text text-xs mt-0.5">{stats.toolCalls}</div>
        </div>
        <div className="text-ash-muted">
          已完成步骤
          <div className="text-ash-text text-xs mt-0.5">{stats.stepDone}</div>
        </div>
        <div className="text-ash-muted">
          执行耗时
          <div className="text-ash-text text-xs mt-0.5">{formatDurationMs(run.startedAt, run.completedAt)}</div>
        </div>
      </div>
      <div className="mt-2 pt-2 border-t border-ash-border/60 text-2xs text-ash-muted flex flex-wrap items-center gap-3">
        ...
        {typeof run.etaSeconds === 'number' && run.etaSeconds > 0 && run.status === 'running' && (
          <span className="text-ash-warning">预计剩余：~{run.etaSeconds}s</span>
        )}
      </div>
    </div>
  );
