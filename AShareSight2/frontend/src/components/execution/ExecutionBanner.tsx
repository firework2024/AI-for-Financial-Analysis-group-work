...
function MiniAgentDot({ status }: { status: AgentRunInfo['status'] }) {
  switch (status) {
    ...
    default:
      return <span className="w-2.5 h-2.5 rounded-full border border-ash-border inline-block" />;
  }
}
    ...
    <div className="flex items-center gap-1.5">
      {all.map((agent) => (
        ...
        >
          ...
          <span className="text-2xs text-ash-muted hidden sm:inline">
          ...
          : isQualityBlocked
            ...
            : isDone
              ...
              : isError
                ...
                : isInterrupted
                  ? 'border-amber-800/30 bg-amber-950/20'
                  : 'border-ash-border bg-ash-bg/50'
      }`}
    >
      <div className="flex items-center gap-3 px-4 py-2">
        {isRunning ? (
          ...
          <StopCircle size={14} className="text-ash-muted shrink-0" />
        )}

        <span className="text-xs text-ash-text font-medium truncate max-w-32">
          {tickerText}
        </span>
        ...
        {isRunning && (
          <div className="flex items-center gap-2 ml-auto shrink-0">
            <div className="w-24 h-1.5 rounded-full bg-ash-border overflow-hidden">
              <div
        ...
        {isRunning && displayRun.currentStep && (
          <span className="text-2xs text-ash-muted truncate max-w-40 hidden md:inline">
            {displayRun.currentStep}
          </span>
        )}

        {!isRunning && (
          <span className="text-2xs text-ash-muted ml-auto shrink-0">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {(isRunning || isInterrupted) && (
            <button
              ...
              className="p-1 rounded hover:bg-red-900/30 text-ash-muted hover:text-red-400 transition-colors"
      ...
      {expanded && displayRun.streamedContent && (
        <div className="px-4 pb-2">
          <div className="text-xs text-ash-text whitespace-pre-wrap break-words max-h-32 overflow-y-auto border border-ash-border rounded-lg p-2 bg-ash-bg/50">
