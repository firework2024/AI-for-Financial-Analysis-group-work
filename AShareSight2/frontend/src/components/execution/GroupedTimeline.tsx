...
function badgeClass(status: TimelineGroup['status']): string {
  ...
  return 'bg-ash-border/60 text-ash-muted';
}
...
export function GroupedTimeline({
  ...
  if (!groups.length) {
    return (
      <div className="rounded-lg border border-ash-border bg-ash-bg/20 px-3 py-3 text-xs text-ash-muted">
        暂无执行时间线
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-ash-border bg-ash-bg/20">
      <div className="flex items-center gap-1.5 px-3 py-2 border-b border-ash-border/60 text-xs text-ash-text-secondary">
        <Clock3 size={12} />
        分组时间线
      </div>
      <div className={`${compact ? 'max-h-48' : 'max-h-72'} overflow-y-auto divide-y divide-ash-border/40`}>
        {groups.map((group) => (
          <div key={group.key} className="px-3 py-2">
            <div className="flex items-center justify-between gap-2">
              <div className="text-xs text-ash-text font-medium truncate">{group.title}</div>
              <div className="flex items-center gap-2 shrink-0">
                <span className={`px-1.5 py-0.5 rounded text-2xs ${badgeClass(group.status)}`}>
                  {group.status}
                </span>
                <span className="text-2xs text-ash-muted">{formatTimelineTime(group.updatedAt)}</span>
              </div>
            </div>
            <div className="mt-1 space-y-1">
              {group.events.map((event) => (
                <div key={event.id} className="text-2xs text-ash-muted leading-relaxed">
