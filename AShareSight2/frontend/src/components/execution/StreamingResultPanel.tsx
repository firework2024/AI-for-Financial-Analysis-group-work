...
export const StreamingResultPanel: React.FC<StreamingResultPanelProps> = ({
  ...
  if (!runId || !run) {
    return (
      <div className={`flex items-center justify-center text-ash-muted text-sm p-8 ${className}`}>
        暂无执行结果
      </div>
    );
  }
      ...
      <div className={`flex flex-col gap-3 ${className}`}>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            <span className="text-ash-muted truncate">
              {run.currentStep ?? '执行中...'}
            </span>
            <span className="text-blue-400 shrink-0">{run.progress}%</span>
          </div>
          <div className="h-1.5 rounded-full bg-ash-border overflow-hidden">
            <div
        ...
        {run.streamedContent && (
          <div
            className={`text-sm text-ash-text whitespace-pre-wrap break-words overflow-y-auto ${
              compact ? 'max-h-48' : 'max-h-96'
            } border border-ash-border rounded-lg p-3 bg-ash-bg/50`}
          >
            {run.streamedContent}
          </div>
        )}
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-ash-muted hover:text-red-400 transition-colors self-start"
        >
          <StopCircle size={14} />
          取消执行
        </button>
      ...
      <div className={`flex flex-col gap-3 ${className}`}>
        {inlineReport ? (
          ...
          <>
            ...
            {run.streamedContent && (
              <div className="text-sm text-ash-text whitespace-pre-wrap break-words overflow-y-auto max-h-96 border border-ash-border rounded-lg p-3 bg-ash-bg/50">
                {run.streamedContent}
              </div>
            )}
          </>
        ...
        {!isDashboardRun && !isQualityBlocked && (
          <div className="flex items-center gap-2 pt-2 border-t border-ash-border/50">
            {isReport ? (
              <button
                type="button"
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg transition-colors ${
                  isBridged
                    ? 'bg-ash-bg text-ash-muted cursor-not-allowed'
                    : 'bg-ash-primary text-white hover:bg-ash-primary/90'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border transition-colors ${
                  isBridged
                    ? 'border-ash-border text-ash-muted cursor-not-allowed'
                    : 'border-ash-border text-ash-text-secondary hover:text-ash-primary hover:border-ash-primary/50'
                }`}
              >
                <Send size={12} />
  ...
  if (run.status === 'error') {
    return (
      <div className={`flex flex-col gap-3 ${className}`}>
        ...
        {run.fallbackReasons.length > 0 && (
          <div className="text-2xs text-ash-muted space-y-0.5">
  ...
  return (
    <div className={`flex items-center gap-2 text-xs text-ash-muted ${className}`}>
      <XCircle size={14} />
      执行已取消
    </div>
  );
