...
function nodeClass(status: PipelineStageState['status'] | undefined, isCurrent: boolean): string {
  ...
  return 'border-ash-border bg-ash-bg text-ash-muted';
}
...
export function PipelineStageBar({
  ...
  return (
    <div className="rounded-lg border border-ash-border bg-ash-bg/30 px-3 py-3">
          ...
          return (
            <div key={stage} className="flex items-center flex-1 min-w-0">
              <div className="flex flex-col items-center gap-1 min-w-[42px]">
                ...
                {!compact && (
                  <span className={`text-2xs ${isCurrent ? 'text-ash-text' : 'text-ash-muted'}`}>
                    {STAGE_LABEL[stage]}
                  </span>
                )}
              </div>
