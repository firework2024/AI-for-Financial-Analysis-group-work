...
class NewsAgent(BaseFinancialAgent):
    ...
    def _recover_original_article_url(self, url: str) -> str:
        ...
        if domain == "rqdatac_news.io" and path.startswith("/api/news"):
    ...
    async def _initial_search(self, query: str, ticker: str) -> List[Any]:
        ...
        # 1) Primary: rqdatac.news.get_stock_news (title + sentiment + source)
        rqdata_news = getattr(self.tools, "get_stock_news", None)
        if rqdata_news and self.circuit_breaker.can_call("rqdatac"):
            try:
                news_items = rqdata_news(ticker=ticker, days=7, max_results=20)
                if isinstance(news_items, list):
                    for item in news_items:
                        ...
                        results.append(item)
                    if results:
                        self.circuit_breaker.record_success("rqdatac")
            except Exception as e:
                logger.info(f"[NewsAgent] rqdatac news failed: {e}")
                self.circuit_breaker.record_failure("rqdatac")

        # 2) Fallback to get_top_news_for_ticker (search-based)
        ...
        # 3) Secondary fallback to web search
        if len(results) < 3 and self.circuit_breaker.can_call("tavily"):
            try:
                search_fn = getattr(self.tools, "search", None)
                if search_fn:
                    t_results = search_fn(f"{ticker} 股票 新闻")
    ...
    async def analyze_stream(self, query: str, ticker: str):
        ...
        if cached:
            ...
            yield json.dumps({
                "type": "source_start",
                "source": "rqdatac",
                "message": "正在检索 RQData 新闻..."
            }, ensure_ascii=False)
            
            if self.circuit_breaker.can_call("rqdatac"):
                try:
                    rqdata_news_fn = getattr(self.tools, "get_stock_news", None)
                    if rqdata_news_fn:
                        news_items = rqdata_news_fn(ticker=ticker, days=7, max_results=20)
                        if news_items:
                            for item in news_items:
                                if isinstance(item, dict):
                                    item["ticker"] = ticker
                ...
                if self.circuit_breaker.can_call("tavily"):
                    try:
                        ...
                        if search_fn:
                            t_results = search_fn(f"{ticker} 股票 新闻")
                            if t_results:
                                # search returns a list of dicts with title/url
                                for item in t_results:
                                    if isinstance(item, dict):
                                        item.setdefault("ticker", ticker)
                                        item.setdefault("source", "search")
