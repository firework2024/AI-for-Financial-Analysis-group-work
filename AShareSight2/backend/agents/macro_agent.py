...
class MacroAgent(BaseFinancialAgent):
    ...
    def _get_tool_registry(self) -> dict:
        ...
        rqdata_macro_fn = getattr(tools, "get_china_macro_factor", None)
        if rqdata_macro_fn:
            registry["get_china_macro_factor"] = {
                "func": rqdata_macro_fn,
                "description": "Fetch Chinese macro indicators (CPI, PPI, PMI, M2, GDP)",
                "call_with": "query",
            }
        ...
        if official_fn:
            registry["get_official_macro_releases"] = {
                "func": official_fn,
                "description": "Get Chinese official macro policy documents",
                "call_with": "query",
            }
    ...
    async def _initial_search(self, query: str, ticker: str) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.tools, "get_china_macro_factor"):
                payload = self.tools.get_china_macro_factor(factor_name="")
                if isinstance(payload, dict):
                    macro_payload = payload
                    macro_metrics = self._extract_numeric_metrics(payload)
                    if macro_metrics:
                        source_health["macro"] = "ok"
                        used_sources.append("macro")
                    else:
                        source_health["macro"] = "empty"
                else:
                    source_health["macro"] = "invalid_payload"
            else:
                source_health["macro"] = "unavailable"
        except Exception as exc:
            source_health["macro"] = f"failed:{exc.__class__.__name__}"
            logger.info("[MacroAgent] rqdatac macro fetch failed: %s", exc)
        ...
        payload: Dict[str, Any] = {
            "status": status,
            "source": "rqdatac" if source_health.get("fred") == "ok" else "search",
